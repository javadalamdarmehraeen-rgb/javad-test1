...
(function () {
  ...
  function fakeFor(path, method) {
    ...
    if (/health|ping|healthz/.test(path)) {
      return jsonResp({ ok: true, status: "healthy", platform: "static-local", version: (window.CRM_APP_VERSION || "12.16.0"), offline: true });
    }
    if (/runtime-config/.test(path)) {
      return jsonResp({ platform: runtime().platform || "static", baseUrl: runtime().baseUrl || "", hubs: runtime().hubs || [], version: "12.16.0" });
    }
