<?php
/**
 * خودبروزرسانیِ نت‌افراز — نسخهٔ 12.23.0
 * -------------------------------------------------------------------------
...
/* ۱) گرفتنِ اطلاعاتِ آخرین release */
...
if (!$j || empty($j["assets"])) { logline("❌ هیچ Release‌ای با فایل ZIP پیدا نشد."); echo "</pre></body></html>"; exit; }
$asset = null;
foreach ($j["assets"] as $a) {
  if (strpos($a["name"], "namayandeelmi-") === 0 && substr($a["name"], -4) === ".zip") { $asset = $a; break; }
}
if (!$asset) { logline("❌ فایل ZIP در release پیدا نشد."); echo "</pre></body></html>"; exit; }
logline("📦 نسخهٔ هدف: " . ($j["tag_name"] ?? "?") . " — " . $asset["name"] . " (" . round($asset["size"] / 1048576, 1) . "MB)");

/* ۲) دانلود */
$tmp = tempnam(sys_get_temp_dir(), "crmzip");
$zipData = @file_get_contents($asset["browser_download_url"], false, $ctx);
if ($zipData === false) { logline("❌ دانلود ZIP ناموفق بود."); echo "</pre></body></html>"; exit; }
