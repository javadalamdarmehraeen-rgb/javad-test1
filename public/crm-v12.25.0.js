/* ============================================================================
   crm-v12.25.0.js — لایهٔ پایانیِ نسخهٔ 12.25.0 (نوبت ۱۵۰)
   ----------------------------------------------------------------------------
   ۲۵) یک ستاره (نه «**نام داروخانه*** *»).
   ۲۶) تابلویِ روان: ۶۰ ثانیه و هر رویداد «یک‌بار».
   ۲۷) تمیزکردنِ موجیبیک + رشتهٔ اقلامِ فشرده و بی‌تکرار.
   ۲۸) کالاها: حذفِ تکراری + سنگ‌قبرِ نامِ حذف‌شده (merge برش نمی‌گرداند).
   ۲۹) آینه‌شدنِ فیلدهایِ «داروخانه‌ها» در «سفارشات».
   ۳۰) فقط یک کادرِ «جایگذاری خودکارِ داروخانه» (بی‌پرش).
   ۳۱) «شمارهٔ ترتیبِ فرم و لیست» می‌ماند (ضبطِ دوبارهٔ لنگر).
   ۳۲) پشتیبانِ خودکار هر ۵ دقیقه.
   ۳۳) «🗺 نمایش تردد» واقعاً نقشه می‌کشد.
   ----------------------------------------------------------------------------
   crm-v12.24.0.js — لایهٔ پایانیِ نسخهٔ 12.24.0 (نوبت ۱۵۰)
   ----------------------------------------------------------------------------
   ۲۱) تابلویِ روان «داخلِ» هدرِ چسبان: با اسکرول همراهِ هدر فریز است.
   ۲۲) پشتیبانِ خودکارِ دیدنی: هوکِ saveState + هر ۵ دقیقه + برچسبِ ساعتِ
...
(function () {
  ...
  var VER = String(window.CRM_APP_VERSION || "12.25.0");
  ...
  /* ───────────────── ۱۴) پشتیبانِ خودکارِ «دیدنی» ─────────────────
     شکایت: «پشتیبان اتوماتیک فعال نیست و باید دستی گرفت». حالا: هر ۵ دقیقه،
  ...
  /* ───────────────── ۱۷) یک ستاره، نه دو ستاره ─────────────────
     شکایت: «فیلدهایِ ستاره‌دار دوستاره‌اند: **نام داروخانه*** *». ریشه: متنِ برچسب در
     رندرهایِ پیاپی ستاره جمع می‌کند و چند spanِ ستاره ساخته می‌شود. حالا در هر گذرِ
     نگهبان، هر «*» داخلِ متنِ برچسب پاک می‌شود و از spanهایِ ستاره فقط «یکی» می‌ماند؛
     خودِ باندل همان یکی را برایِ فیلدِ اجباری می‌سازد. */
  function fixStars1225() {
    var n = 0;
    try {
      var labs = document.querySelectorAll(".form-group label, .form-group .form-label");
      Array.prototype.forEach.call(labs, function (lab) {
        if (!lab || !lab.childNodes) return;
        var changed = false;
        for (var i = 0; i < lab.childNodes.length; i++) {
          var nd = lab.childNodes[i];
          if (nd && nd.nodeType === 3 && String(nd.nodeValue || "").indexOf("*") >= 0) {
            nd.nodeValue = String(nd.nodeValue).replace(/\s*\*+\s*/g, " ");
            changed = true;
          }
        }
        var stars = (lab.querySelectorAll && lab.querySelectorAll(".req-star")) || [];
        for (var k = 1; k < stars.length; k++) {
          try { stars[k].parentNode.removeChild(stars[k]); changed = true; } catch (e) {}
        }
        if (changed) n++;
      });
    } catch (e) {}
  ...
  /* ───────────────── ۱۸) تمیزکردنِ متنِ فارسی (موجیبیک) و جمع‌کردنِ رشتهٔ اقلام ─────────────────
     شکایت: نام‌ها به‌صورتِ «â€Žâ€Ž…» و رشتهٔ «سافت ژل امگا 3 = تعداد کالا: 100 / تعداد
     جایزه: 10، …» با تکرارِ نام دیده می‌شود. حالا نویزِ کدگذاری پاک می‌شود و هر نام
     «یک‌بار» و فشرده نمایش داده می‌شود. */
  function cleanFaText1225(s) {
    var t = String(s == null ? "" : s);
    /* نویزِ کدگذاری: U+FFFD، نویزهایِ کنترلی و نشانه‌هایِ جهت */
    t = t.replace(/[\uFFFD\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, "");
    t = t.replace(/[\u200E\u200F\u202A-\u202E\u2066-\u2069]/g, "");
    /* نویزهایِ کنترلیِ C1 */
    t = t.replace(/[\u0080-\u009F]/g, "");
    /* الفبایِ موجیبیک: دنباله‌هایِ «â€Ž»، «Ã‚Â»، «ØŒ»، «ÛŒ» و مانندِ آن.
       فقط همین حرف‌ها پاک می‌شوند — ««»» و «®» و «°» و خط‌تیره‌هایِ فارسی دست‌نخورده
       می‌مانند چون در متنِ سالم به کار می‌روند. */
    t = t.replace(/[\u00C2\u00C3\u00D8\u00DB\u00E2\u0152\u0153\u0160\u0161\u0178\u017D\u017E\u0192\u02C6\u02DC\u201A\u201E\u2020\u2021\u2030\u2039\u203A\u20AC\u2122]/g, "");
    /* فاصلهٔ نشکن → فاصلهٔ معمولی، سپس جمع‌کردنِ فاصله‌هایِ پیاپی */
    t = t.replace(/[\u00A0\t]/g, " ").replace(/ {2,}/g, " ");
    return t.trim();
  }
  function fixGiftCells1225() {
    var fixed = 0;
    try {
      var pane = $("tab-orders");
      if (!pane || !pane.querySelectorAll) return 0;
      var tds = pane.querySelectorAll("td");
      Array.prototype.forEach.call(tds, function (td) {
        var raw = String(td.textContent || "");
        if (raw.indexOf("= تعداد کالا:") < 0) return;
        var seen = {}, parts = [];
        raw.split("،").forEach(function (seg) {
          var m = String(seg).match(/^\s*([^=]+?)\s*=\s*تعداد کالا:\s*([\d\u06F0-\u06F9]+)\s*\/\s*تعداد جایزه:\s*([\d\u06F0-\u06F9]+)/);
          if (!m) return;
          var name = cleanFaText1225(m[1]);
          if (!name || seen[name]) return;
          seen[name] = 1;
          parts.push(name + " (" + cleanNum(m[2]) + " کالا / " + cleanNum(m[3]) + " جایزه)");
        });
        if (!parts.length) return;
        var txt = parts.join("، ");
        if (td.textContent !== txt) { td.textContent = txt; fixed++; }
      });
    } catch (e) {}
    return fixed;
  }

  /* ───────────────── ۱۹) کالاها: حذفِ تکراری + سنگ‌قبرِ حذفِ کاربر ─────────────────
     شکایت: «نام کالاها دوبار تکرار شده و هر چه در تبِ ستون‌ها و کالاها حذف می‌کنم
     مجدداً ظاهر می‌شود، مخصوصاً در رندر». ریشه: mergeِ سرور رکوردهایِ حذف‌شده را
     برمی‌گرداند. حالا نامِ حذف‌شده سنگ‌قبر می‌گیرد و دیگر برنمی‌گردد؛ تکراری‌ها هم
     بر پایهٔ نامِ نرمال‌شده یکی می‌شوند. */
  var PROD_TOMB1225 = "CRM_V1225_PRODUCT_TOMB";
  var v1225PrevNames = null;
  function normName1225(s) { return cleanFaText1225(s).replace(/\s+/g, " ").toLowerCase(); }
  function productGuard1225() {
    var S = window.state;
    if (!S) return 0;
    var list = S.products;
    if (!list || !list.length) { v1225PrevNames = []; return 0; }
    var tomb = readJson(PROD_TOMB1225, {}) || {};
    var seen = {}, out = [], dropped = 0;
    list.forEach(function (p) {
      var nm = normName1225(p && p.name);
      if (!nm) { out.push(p); return; }
      if (tomb[nm] || seen[nm]) { dropped++; return; }
      seen[nm] = 1; out.push(p);
    });
    if (dropped) { S.products = out; saveSoft(); }
    var now = Object.keys(seen);
    if (v1225PrevNames) {
      var dirty = false;
      v1225PrevNames.forEach(function (nm) {
        if (now.indexOf(nm) < 0 && !tomb[nm]) { tomb[nm] = Date.now(); dirty = true; }
      });
      if (dirty) writeJson(PROD_TOMB1225, tomb);
    }
    v1225PrevNames = now;
    return dropped;
  }

  /* ───────────────── ۲۰) هر فیلدی که در «داروخانه‌ها» افزوده می‌شود، در «سفارشات» هم باشد ─────────────────
     شکایت: «هر فیلدی در تبِ داروخانه‌ها افزوده می‌شود باید در تبِ سفارشات هم به همان
     شکل اضافه شود». حالا فیلدهایِ سفارشیِ pharmacy به order آینه می‌شوند (همان
     عنوان/نوع/گزینه‌ها/ترتیب/نمایش) و اگر فیلدِ مبدأ حذف شد، آینه‌اش هم می‌رود. */
  function mirrorPharmacyToOrders1225() {
    var S = window.state;
    if (!S) return 0;
    S.customFields = S.customFields || {};
    var src = S.customFields["pharmacy"] || [];
    var dst = S.customFields["order"] = (S.customFields["order"] || []);
    var added = 0, removed = 0, i;
    for (i = dst.length - 1; i >= 0; i--) {
      var d = dst[i];
      if (!d || !d.mirrorOf) continue;
      var alive = src.some(function (s) { return s && s.id === d.mirrorOf; });
      if (!alive) { dst.splice(i, 1); removed++; }
    }
    src.forEach(function (s) {
      if (!s || !s.id) return;
      var lbl = String(s.label || "").trim();
      var exists = dst.some(function (x) {
        return x && (x.mirrorOf === s.id || String(x.label || "").trim() === lbl);
      });
      if (exists) return;
      dst.push({
        id: "cf-order-mir-" + String(s.id).replace(/[^a-zA-Z0-9]/g, "").slice(-8) + "-" + Date.now().toString(36).slice(-4),
        label: s.label,
        type: s.inputKind || s.type || "simple",
        inputKind: s.inputKind || s.type || "simple",
        options: (s.options || []).slice(),
        order: s.order, listOrder: s.listOrder, size: s.size, height: s.height,
        gapBeforeMm: s.gapBeforeMm, gapAfterMm: s.gapAfterMm, rowNo: s.rowNo,
        showInForm: s.showInForm !== false, showInList: s.showInList !== false,
        place: s.place || "beside", required: !!s.required, mirrorOf: s.id
      });
      added++;
    });
    if (added || removed) {
      ...
      try { if (typeof window.refreshColumnsDesigner === "function") window.refreshColumnsDesigner(); } catch (e) {}
    }
    return added;
  }

  /* ───────────────── ۲۱) فقط یک کادرِ «جایگذاری خودکارِ داروخانه» ─────────────────
     شکایت: «در تبِ سفارشات دو کادرِ جایگذاری خودکارِ داروخانه هست و یکی‌شان مدام
     حذف و ظاهر می‌شود و پرش دارد». ریشه: دو سازنده (#crm1215OrderPick و
     #crm1216TopPick) هر دو کادر را در tab.firstChild می‌کارند؛ حذف‌کردن هم فایده
     ندارد چون سازنده دوباره می‌سازد. پس کادرِ دوم «پنهان» می‌ماند (نه حذف) تا
     سازنده‌اش ساکت بماند و پرش تمام شود. */
  function singlePickBox1225() {
    try {
      var keep = $("crm1215OrderPick");
      var other = $("crm1216TopPick");
      if (!keep || !other) return 0;
      if (other.style.getPropertyValue("display") !== "none") {
  ...
  /* ───────────────── ۲۲) «شمارهٔ ترتیبِ فرم و لیست» واقعاً بماند ─────────────────
     شکایت: «متأسفانه دوباره شمارهٔ ترتیبِ فرم و لیست کار نمی‌کند». ریشه: نگهبانِ
     لایه هر ۹۰۰ms «ترتیبِ لنگرِ» ضبط‌شده در آغاز را دوباره اعمال می‌کند، پس شمارهٔ
     تازه را در کمتر از یک ثانیه به جایِ اول برمی‌گرداند. حالا پس از هر تغییرِ
     شماره، لنگرِ «همان تب» دوباره ضبط می‌شود تا نگهبان ترتیبِ تازه را نگه دارد. */
  function recaptureAfterOrder1225(tabId) {
    var t = tabId || window._activeColTab || "tab-pharmacies";
    setTimeout(function () { try { captureCanon(t); } catch (e) {} }, 1100);
    setTimeout(function () { try { captureCanon(t); } catch (e) {} }, 2500);
  }

  /* ───────────────── ۲۳) «🗺 نمایش تردد» واقعاً نقشه بکشد ─────────────────
     شکایت: «کلیدِ نمایش تردد در تبِ رصد تردد هیچ عکس‌العملی ندارد». ریشه‌ها: نقشه
     هنوز ساخته نشده بود (L.map روی کادرِ پنهان)، مسیر از سطر پیدا نمی‌شد، و پیام‌ها
     در alert خاموش می‌ماندند. حالا لایه خودش کلیک را می‌گیرد، تب را باز می‌کند،
     نقشه را می‌سازد، مسیر را می‌کشد و وضعیت را «دیدنی» می‌نویسد. */
  function trafficSay1225(t) {
    var pane = $("tab-rep-routes");
    var box = $("v1225TrafficStatus");
    if (!box && pane) {
      box = document.createElement("div");
      box.id = "v1225TrafficStatus";
      box.className = "v1225-traffic-status";
      try { pane.insertBefore(box, pane.firstChild); } catch (e) { try { pane.appendChild(box); } catch (e2) {} }
    }
    if (box && box.textContent !== t) box.textContent = t;
  }
  function findTrack1225(txt) {
    var S = window.state;
    if (!S) return null;
    var tracks = [].concat(S.visitTracks || [], S.repRoutes || []);
    if (!tracks.length) return null;
    var s = String(txt || "");
    var byName = tracks.filter(function (t) { return t && t.repName && s.indexOf(String(t.repName)) >= 0; });
    if (byName.length) return byName[0];
    var byDate = tracks.filter(function (t) { return t && t.date && s.indexOf(String(t.date)) >= 0; });
    return byDate[0] || tracks[0] || null;
  }
  function openRoutesTab1225() {
    var tab = $("tab-rep-routes");
    if (!tab) return;
    var active = tab.classList && tab.classList.contains && tab.classList.contains("active");
    if (active) return;
    try {
      var fn = window.showTab || window.switchTab || window.openTab || window.activateTab;
      if (typeof fn === "function") { fn("tab-rep-routes"); return; }
      var link = document.querySelector("[data-tab='tab-rep-routes'],[href='#tab-rep-routes'],[data-target='tab-rep-routes']");
      if (link && link.click) link.click();
    } catch (e) {}
  }
  function ensureRoutesMap1225() {
    try {
      var m = window._mapRepRoutes;
      if (m && typeof m.invalidateSize === "function") return m;
      if (typeof window.L === "undefined") return null;
      var host = $("map-rep-routes-full") || $("mapRepRoutes") ||
        (document.querySelector && document.querySelector("#tab-rep-routes [id*=map],#tab-rep-routes .map-box"));
      if (!host) return null;
      var map = window.L.map(host.id || host).setView([35.73, 51.42], 12);
      try {
        (window.crmAddMapTiles || function (mm) {
          window.L.tileLayer("/api/tiles/{z}/{x}/{y}.png", { maxZoom: 19 }).addTo(mm);
        })(map);
      } catch (e) {}
  ...
  function showTraffic1225(tr) {
    openRoutesTab1225();
    var txt = String((tr && tr.textContent) || "");
    var track = null;
    try {
      if (window.v1215Api && typeof window.v1215Api.findTrackForRow === "function") track = window.v1215Api.findTrackForRow(txt);
    } catch (e) {}
    if (!track) track = findTrack1225(txt);
    if (!track) {
      trafficSay1225("⚠️ برای این سطر مسیری ثبت نشده — نخست در «شروع/پایان ویزیت» یک جلسه ثبت کنید.");
      try { toast("🗺 مسیری برای این سطر پیدا نشد."); } catch (e) {}
      return false;
    }
    var pts = (track.path && track.path.length) ? track.path
      : (track.points || []).map(function (p) { return [p.lat, p.lng]; });
    pts = pts.filter(function (p) { return p && isFinite(p[0]) && isFinite(p[1]); });
    if (pts.length < 2) {
      trafficSay1225("⚠️ این مسیر کمتر از دو نقطه دارد («" + (track.repName || "—") + "») — چیزی برای رسم نیست.");
      try { toast("🗺 مسیرِ این سطر نقطهٔ کافی ندارد."); } catch (e) {}
      return false;
    }
    var map = ensureRoutesMap1225();
    if (!map) {
      trafficSay1225("⚠️ نقشه آماده نشد — یک‌بار تبِ «رصد تردد» را باز کنید تا برگهٔ نقشه بارگیری شود.");
      try { toast("🗺 نقشه آماده نشد."); } catch (e2) {}
      return false;
    }
    try {
      if (window.__v1225Line) { try { map.removeLayer(window.__v1225Line); } catch (e) {} }
      (window.__v1225Marks || []).forEach(function (mk) { try { map.removeLayer(mk); } catch (e) {} });
      var line = window.L.polyline(pts, { color: "#0d9488", weight: 4 }).addTo(map);
      var mkA = window.L.marker(pts[0]).addTo(map); try { mkA.bindPopup("مبدأ — " + (track.repName || "")); } catch (e) {}
      var mkB = window.L.marker(pts[pts.length - 1]).addTo(map); try { mkB.bindPopup("مقصد"); } catch (e) {}
      window.__v1225Line = line;
      window.__v1225Marks = [mkA, mkB];
      try { map.fitBounds(line.getBounds(), { padding: [30, 30] }); } catch (e) {}
      setTimeout(function () { try { map.invalidateSize(); } catch (e) {} }, 250);
    } catch (e) {
      trafficSay1225("❌ رسمِ مسیر ناموفق بود: " + (e && e.message ? e.message : ""));
      return false;
    }
    trafficSay1225("✅ ترددِ «" + (track.repName || "نماینده") + "» با " + pts.length + " نقطه و " +
      Math.round(Number(track.distance) || 0) + " متر روی نقشه رسم شد.");
    try { toast("🗺 تردد روی نقشه رسم شد."); } catch (e) {}
    return true;
  }
  ...
  function tickerText() {
    ...
    /* نوبتِ ۱۵۲: هیچ رویدادی دوبار نیاید */
    out = out.filter(function (x, i) { return out.indexOf(x) === i; });
    return out.join("  ✦  ");
  }
  ...
  function buildTicker() {
    try {
      ...
      if (!bar) {
        ...
        /* نوبتِ ۱۵۲: فقط «یک» متن — هر رویداد یک‌بار دیده می‌شود (نه دوبار) */
        bar.innerHTML = '<div class="crm-ticker-label">🗓 رویدادهای امروز</div><div class="crm-ticker-view"><div class="crm-ticker-move"><span id="crmTickerText"></span></div></div>';
      }
      ...
      /* نسخهٔ قدیمی یک کپیِ دوم برایِ حلقهٔ بی‌درز می‌ساخت → هر رویداد دوبار دیده
         می‌شد. حالا کپیِ دوم پاک می‌شود و فقط یک متن می‌ماند. */
      var dup = document.getElementById("crmTickerText2");
      if (dup && dup.parentNode) { try { dup.parentNode.removeChild(dup); } catch (e) {} }
      var txt = tickerText();
      var a = document.getElementById("crmTickerText");
  ...
  function boot() {
    ...
    /* تغییرِ «شمارهٔ ترتیبِ فرم و لیست» — نوبتِ ۱۵۲: نگهبان ترتیبِ تازه را نگه دارد
       (پیش‌تر rebuildAllCanons لنگرِ تازه می‌گرفت و شماره را بی‌اثر می‌کرد) */
    try {
      document.addEventListener("change", function (e) {
        var t = e.target;
        if (!t) return;
        var isOrder = t.classList && (t.classList.contains("col-order-input") || t.classList.contains("col-listorder-input"));
        if (!isOrder && t.id !== "colFieldOrder" && t.id !== "colFieldListOrder") return;
        recaptureAfterOrder1225(window._activeColTab || "tab-pharmacies");
        setTimeout(function () { try { pass(); } catch (e1) {} }, 1400);
      }, true);
    } catch (eOR) {}
    ...
    /* ── نوبتِ ۱۵۲: نگهبان‌هایِ تازه ─────────────────────────────────────── */
    try {
      setInterval(function () {
        try { singlePickBox1225(); } catch (e1) {}
        try { productGuard1225(); } catch (e2) {}
        try { fixGiftCells1225(); } catch (e3) {}
      }, 1200);
      setInterval(function () { try { mirrorPharmacyToOrders1225(); } catch (e4) {} }, 4000);
      setTimeout(function () { try { mirrorPharmacyToOrders1225(); } catch (e5) {} }, 900);
      setTimeout(function () { try { singlePickBox1225(); } catch (e6) {} }, 900);
    } catch (eG25) {}

    /* «🗺 نمایش تردد»: لایه خودش کلیک را می‌گیرد و نقشه را واقعاً می‌کشد */
    try {
      document.addEventListener("click", function (e) {
        var t = e.target;
        if (!t || !t.closest) return;
        var btn = t.closest(".crm1215-traffic-btn,#btnV20ShowRoute,.v20-traffic-btn");
        if (!btn) return;
        try { e.preventDefault(); e.stopImmediatePropagation(); } catch (e2) {}
        showTraffic1225(btn.closest("tr") || null);
      }, true);
    } catch (eTB) {}
    ...
    /* پشتیبانِ خودکارِ «دیدنی»: هر ۵ دقیقه اگر چیزی عوض شده بود (نوبتِ ۱۵۲) */
    setInterval(function () {
      try {
        var prev = readJson(VAULT_KEY, null);
        var snap = vaultSnapshot();
  ...
  window.v1220Api = {
    ...
    fixStars1225: fixStars1225,
    cleanFaText1225: cleanFaText1225,
    fixGiftCells1225: fixGiftCells1225,
    productGuard1225: productGuard1225,
    mirrorPharmacyToOrders1225: mirrorPharmacyToOrders1225,
    singlePickBox1225: singlePickBox1225,
    recaptureAfterOrder1225: recaptureAfterOrder1225,
    showTraffic1225: showTraffic1225,
    findTrack1225: findTrack1225,
    ensureRoutesMap1225: ensureRoutesMap1225,
    trafficSay1225: trafficSay1225
  };
