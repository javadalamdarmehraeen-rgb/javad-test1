...
(function () {
  ...
  function selectOrderPharmacyCardV37(card){var S=st(),id=card&&card.getAttribute("data-pid"),rec=((S&&S.pharmacies)||[]).filter(function(p){return String(p.id)===String(id);})[0];if(!rec)return false;v20SetValue("orderPharmacyName",rec.name||"");v20SetValue("orderPharmacyMatchedId",rec.id||"");var inp=$("orderPharmacyName");if(inp)inp.dataset.v20PlacedName=rec.name||"";v20FillOrderPharmacy();hidePlacedOrderNotices();return true;}
  function bindOrderPharmacyCardV37(){if(document.documentElement.dataset.v37phcards)return;document.documentElement.dataset.v37phcards="1";document.addEventListener("click",function(e){var card=e.target&&e.target.closest&&e.target.closest("#orderPharmacyPickBox .ph-pick-card");if(!card)return;e.preventDefault();e.stopImmediatePropagation();selectOrderPharmacyCardV37(card);},true);}
  ...
  function setupV37FinalGuards(){enforceDeletedUserTombstonesV37();installPersianBuiltinLabelGuardV37();setupPlainRecipientV37();bindComboCaretFixV37();bindOrderPharmacyCardV37();pinRepresentativeRouteFieldsV37();window.renderRepHomesTable=renderRepHomesV37;renderRepHomesV37();}
