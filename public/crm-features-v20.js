...
(function () {
  ...
  function setupChangeLogTabV41(){var pane=$("tab-changelog");if(!pane){pane=document.createElement("div");pane.className="tab-pane";pane.id="tab-changelog";pane.innerHTML="<div class='card'><div class='card-header'><div class='card-title'>📋 تغییرات در نسخه جدید (v11.41.0)</div></div><p class='col-help'>ریز تغییرات نسبت به نسخه قبل + چک‌باکس «اعمال نشده». تیک بزنید و گزارش بفرستید تا جلسه بعد علت‌یابی شود.</p><div id='v41ChangeHost'></div><div style='display:flex;gap:8px;margin-top:12px;flex-wrap:wrap'><button type='button' id='btnSendUnappliedReport' class='btn btn-primary'>📤 ارسال گزارش اعمال نشده‌ها</button><button type='button' id='btnDownloadUnappliedReport' class='btn btn-outline'>⬇️ دانلود فایل گزارش</button></div><div id='v41ReportStatus' style='margin-top:8px;font-size:12px;color:#0f766e'></div></div>";var content=document.querySelector(".tab-content")||document.querySelector("main");if(content)content.appendChild(pane);}var nav=document.querySelector(".app-nav, .side-menu-drawer, nav");if(nav&&!$("navtabchangelog")){var a=document.createElement("button");a.type="button";a.id="navtabchangelog";a.className="nav-item";a.setAttribute("data-target","tab-changelog");a.innerHTML="📋 تغییرات در نسخه جدید";a.onclick=function(){try{window.switchTab("tab-changelog");}catch(e){}};nav.appendChild(a);}renderChangeLogV41();var sb=$("btnSendUnappliedReport"),dl=$("btnDownloadUnappliedReport");if(sb&&!sb.dataset.b){sb.dataset.b="1";sb.onclick=sendUnappliedReportV41;}if(dl&&!dl.dataset.b){dl.dataset.b="1";dl.onclick=function(){var r=collectUnappliedV41();var b=new Blob([JSON.stringify(r,null,2)],{type:"application/json"});var u=URL.createObjectURL(b),a2=document.createElement("a");a2.href=u;a2.download="unapplied-changes-11.41.0.json";a2.click();URL.revokeObjectURL(u);};}}

  /* ---------- v11.42 / turn 74: اعمال گزارش کاربر — تایید/تکراری/پاک‌سازی/قانون نماینده ---------- */
  ...
  function setupV37FinalGuards(){setupV42Guards();setupChangeLogTabV41();enforceDeletedUserTombstonesV37();enforceRemovedIdentitiesV40();installReferenceInstantAddGuardV40();installPersianBuiltinLabelGuardV37();setupPlainRecipientV37();bindComboCaretFixV37();bindOrderPharmacyCardV37();pinRepresentativeRouteFieldsV37();window.renderRepHomesTable=renderRepHomesV37;renderRepHomesV37();}
