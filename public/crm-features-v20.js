...
(function () {
  ...
  function permanentlyRemovedNamesV40(){var t=((st()&&st().settings&&st().settings.deletedUserTombstones)||[]),names={},usern={},ids={};t.forEach(function(x){if(x&&x.fullName)names[norm(x.fullName)]=1;if(x&&x.username)usern[String(x.username).toLowerCase()]=1;if(x&&x.id)ids[String(x.id)]=1;});["جواد علمدار","خانم نیلا محرمی"].forEach(function(n){names[norm(n)]=1;});usern["taheri"]=1;usern["nila"]=1;ids["u-2"]=1;ids["u-3"]=1;return{names:names,usern:usern,ids:ids};}
  function enforceRemovedIdentitiesV40(){var S=st();if(!S)return;var m=permanentlyRemovedNamesV40(),before=(S.users||[]).length;S.users=(S.users||[]).filter(function(u){return u&&!m.ids[String(u.id)]&&!m.usern[String(u.username||"").toLowerCase()]&&!m.names[norm(u.fullName||"")];});if(S.users.length!==before){try{syncUsersAuthV27();syncRepsFromUsers();saveState(false);}catch(e){}}}
  function dropRemovedUserRowsV40(rows){var m=permanentlyRemovedNamesV40();return (rows||[]).filter(function(r){var n=norm(r&&(r.repName||r.name||r.rep||""));return !n||!m.names[n];});}
