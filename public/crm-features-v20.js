...
(function () {
  ...
  window.v20RenderComboManager = function () {
    ...
    var h = "<div class='v20-addmgr' style='border-top:2px solid #e2e8f0;padding-top:10px;'>" +
      "<h4 style='margin:8px 0;color:#0f172a;'>🎛️ مدیر کشویی‌ها (نسخه ۱۱.۲۱.۰) — هر فیلد، زیرمجموعه‌هایش دقیقاً زیر همان فیلد است</h4>" +
  ...
  function renderVersionBadge() {
    ...
    if (!b) { b = document.createElement("span"); b.id = "v20VersionBadge"; b.className = "v20-version"; b.textContent = "نسخه ۱۱.۲۱.۰"; b.title = "نسخه دقیق برنامه نصب‌شده"; actions.insertBefore(b, actions.firstChild); }
  ...
  function rowSignature(r){return(r||[]).map(function(x){return enDigits(String(x==null?"":x)).replace(/\s+/g," ").trim();}).join("¦");}
  ...
  var DIST_DEFS=[["daya","دایا دارو"],["shafaarad","شفاآراد"],["tivan","تیوان"],["mashateb","مشاطب"]],DIST_HEADERS=["نام کالا","فروش تعدادی","فروش ریالی به قیمت پخش","فروش ریالی به قیمت داروخانه","تعداد جایزه","ریال جایزه","درصد جایزه به فروش","تعداد مرجوعی","ریال مرجوعی","درصد مرجوعی به فروش","تعداد داروخانه","تعداد فاکتور","درصد فروش به‌تفکیک کالا","موجودی تعدادی","موجودی ریالی"],distReportCache={};
  ...
  function distSchema(h){return{date:findDistIndex(h,/تاریخ/,0),product:findDistIndex(h,/نام.*(کالا|محصول)|کالا|محصول/,1),qty:findDistIndex(h,/فروش.*تعداد|تعداد.*فروش/,2),dist:findDistIndex(h,/فروش.*ریال.*پخش|مبلغ.*پخش/,3),ph:findDistIndex(h,/فروش.*ریال.*داروخانه|مبلغ.*داروخانه/,4),giftQty:findDistIndex(h,/تعداد.*جایزه/,5),giftRial:findDistIndex(h,/ریال.*جایزه|مبلغ.*جایزه/,6),retQty:findDistIndex(h,/تعداد.*مرجوع/,7),retRial:findDistIndex(h,/ریال.*مرجوع|مبلغ.*مرجوع/,8),pharmacy:findDistIndex(h,/نام.*داروخانه|داروخانه/,9),invoice:findDistIndex(h,/شماره.*فاکتور|فاکتور/,10)};}
  function invSchema(h){return{product:findDistIndex(h,/نام.*(کالا|محصول)|کالا|محصول/,0),qty:findDistIndex(h,/موجودی.*تعداد|تعداد.*موجودی/,1),rial:findDistIndex(h,/موجودی.*ریال|ریال.*موجودی/,2)};}
  ...
  function emptyMetric(){return{qty:0,dist:0,ph:0,giftQty:0,giftRial:0,retQty:0,retRial:0,pharmacies:{},invoices:{},invQty:0,invRial:0};}
  function distributorMetrics(d){var f=distFilter(),s=distSchema(d.pharmacyHeaders),is=invSchema(d.inventoryHeaders),map={};(d.pharmacyRows||[]).forEach(function(r){if(!distDatePass(r[s.date],f))return;var n=String(r[s.product]||"نامشخص").trim()||"نامشخص",m=map[n]||(map[n]=emptyMetric());m.qty+=snappNumber(r[s.qty]);m.dist+=snappNumber(r[s.dist]);m.ph+=snappNumber(r[s.ph]);m.giftQty+=snappNumber(r[s.giftQty]);m.giftRial+=snappNumber(r[s.giftRial]);m.retQty+=snappNumber(r[s.retQty]);m.retRial+=snappNumber(r[s.retRial]);if(s.pharmacy>=0&&r[s.pharmacy])m.pharmacies[String(r[s.pharmacy])]=1;if(s.invoice>=0&&r[s.invoice])m.invoices[String(r[s.invoice])]=1;});(d.inventoryRows||[]).forEach(function(r){var n=String(r[is.product]||"نامشخص").trim()||"نامشخص",m=map[n]||(map[n]=emptyMetric());m.invQty+=snappNumber(r[is.qty]);m.invRial+=snappNumber(r[is.rial]);});((st().products)||[]).forEach(function(p){var n=p.name||"";if(n&&!map[n])map[n]=emptyMetric();});return map;}
  function mergeMetricMaps(target,src){Object.keys(src).forEach(function(n){var a=target[n]||(target[n]=emptyMetric()),b=src[n];["qty","dist","ph","giftQty","giftRial","retQty","retRial","invQty","invRial"].forEach(function(k){a[k]+=b[k]||0;});Object.assign(a.pharmacies,b.pharmacies);Object.assign(a.invoices,b.invoices);});return target;}
  function metricRows(map){var totalSales=Object.values(map).reduce(function(s,m){return s+m.dist;},0);return Object.keys(map).sort().map(function(n){var m=map[n],giftPct=m.dist?m.giftRial/m.dist*100:0,retPct=m.dist?m.retRial/m.dist*100:0,share=totalSales?m.dist/totalSales*100:0;return[n,m.qty,m.dist,m.ph,m.giftQty,m.giftRial,giftPct,m.retQty,m.retRial,retPct,Object.keys(m.pharmacies).length,Object.keys(m.invoices).length,share,m.invQty,m.invRial];});}
  function totalMetricRow(rows){var t=["جمع کل"];for(var c=1;c<15;c++){if([6,9,12].indexOf(c)>=0){var base=c===6?5:c===9?8:2,den=rows.reduce(function(s,r){return s+Number(r[2]||0);},0),num=rows.reduce(function(s,r){return s+Number(r[base]||0);},0);t[c]=den?num/den*100:0;}else t[c]=rows.reduce(function(s,r){return s+Number(r[c]||0);},0);}return t;}
  function displayMetric(v,i){if(i===0)return esc(v);if([6,9,12].indexOf(i)>=0)return Number(v||0).toLocaleString("fa-IR",{maximumFractionDigits:2})+"٪";return Number(v||0).toLocaleString("fa-IR");}
  ...
  function refreshDistFilterLock(){var y=String(($("distFilterYear")||{}).value||"").trim(),m=$("distFilterMonth"),day=$("distFilterDay");if(m){m.disabled=!y;m.classList.toggle("v20-grey",!y);if(!y){m.value="";$("distModeMonth").checked=false;}}var off=!y||!String((m||{}).value||"");if(day){day.disabled=off;day.classList.toggle("v20-grey",off);if(off){day.value="";$("distModeDay").checked=false;}}}
  ...
  function init() {
    ...
    try { renderDistributorCompanies(); setupDistributorSales(); } catch (e) {}
