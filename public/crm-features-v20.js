...
(function () {
  ...
  window.v20RenderComboManager = function () {
    ...
    var h = "<div class='v20-addmgr' style='border-top:2px solid #e2e8f0;padding-top:10px;'>" +
      "<h4 style='margin:8px 0;color:#0f172a;'>🎛️ مدیر کشویی‌ها (نسخه ۱۱.۲۰.۲) — هر فیلد، زیرمجموعه‌هایش دقیقاً زیر همان فیلد است</h4>" +
  ...
  function renderVersionBadge() {
    ...
    if (!b) { b = document.createElement("span"); b.id = "v20VersionBadge"; b.className = "v20-version"; b.textContent = "نسخه ۱۱.۲۰.۲"; b.title = "نسخه دقیق برنامه نصب‌شده"; actions.insertBefore(b, actions.firstChild); }
  ...
  function bindDurableServerState(){var old=window.saveState;if(typeof old!=="function"||old._v20durable)return;var timer;var w=function(){var r=old.apply(this,arguments);clearTimeout(timer);timer=setTimeout(function(){var S=st();if(!S)return;fetch("/api/state",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(S)}).catch(function(){});},1500);return r;};w._v20durable=true;window.saveState=w;
    if(sessionStorage.getItem("v20RemoteRecoveryChecked")==="1")return;sessionStorage.setItem("v20RemoteRecoveryChecked","1");fetch("/api/state").then(function(r){return r.json();}).then(function(res){var remote=res&&res.data;if(!remote||typeof remote!=="object")return;var local=st()||{},use=!window.__CRM_HAD_SAVED_STATE||Number(remote._lastSavedAt||0)>Number(local._lastSavedAt||0);if(use){localStorage.setItem("CRM_APP_STATE_V2",JSON.stringify(remote));localStorage.setItem("CRM_APP_STATE_BACKUP_LATEST",JSON.stringify(remote));location.reload();}}).catch(function(){});}
