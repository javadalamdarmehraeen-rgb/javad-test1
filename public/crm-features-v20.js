...
(function () {
  ...
  function v20ReorderListColumns(paneId, key, headId, bodyId) {
    try {
      ...
      function reorderRow(row) {
        ...
        // v11.61.0: فقط وقتی ترتیب واقعاً متفاوت است DOM جابه‌جا شود؛ جابه‌جایی تکراریِ همان ترتیب، حلقه خودتغذی MutationObserver می‌ساخت
        var needs = false;
  ...
  function restoreOrderFormSequence(snapshot){var form=$("formOrder");if(!form||!snapshot)return;var grids=Array.prototype.slice.call(form.querySelectorAll(".form-grid"));snapshot.forEach(function(rec,i){var grid=rec.key&&$(rec.key)||grids[i];if(!grid)return;var groups={};Array.prototype.forEach.call(grid.children,function(g){var id=groupAnchor(g);if(id)groups[id]=g;});
    // v11.61.0: در صورت مطابقت ترتیب فعلی با اسنپ‌شات، هیچ appendChild انجام نشود (جلوگیری از حلقه خودتغذی MutationObserver)
  ...
  function bindOrderFormPositionLock(){var form=$("formOrder");if(!form||form.dataset.v36positionLock)return;form.dataset.v36positionLock="1";var stable=null,busy=false,timer=0;function approve(){stable=captureOrderFormSequence();}form.addEventListener("crm-order-layout-approved",approve);setTimeout(function(){restoreDomFieldOrder();approve();},1900);if(window.MutationObserver)new MutationObserver(function(records){if(busy||!stable||form.dataset.v36Resetting==="1")return;if(window.__CRM_MANAGER_LAYOUT_INTENT){clearTimeout(timer);timer=setTimeout(approve,900);return;}
    // v11.61.0: فقط تغییرات ساختاری واقعی (نه بازشدن لیست کشویی/تایپ) موجب بازنشانی ترتیب شود؛ busy آسنکرون آزاد می‌شود تا ناظر خودتغذی حلقه نسازد
  ...
  var V41_CHANGES=[
    ...
    ["ستون شماره همراه لیست داروخانه مقدار درست (همراه مسئول) نشان دهد","partial"],
    ["پزشکان: عیناً همان اصلاحات تب داروخانه","pending"],
    ["سفارشات: فیلد اولویت ارسال (عادی/فوری) + فعال‌بودن فقط تاریخ/داروخانه/اولویت و بقیه طوسی غیرفعال ساده","pending"],
    ...
    ["شرکت‌ها: ماندن اطلاعات در فیلدها + قابل ویرایش + نمایش/پنهان رمزها در کل برنامه","pending"],
  ...
  function collectUnappliedV41(){var v="11.42.0";try{var m=document.querySelector('script[src*="crm-features-v20.js?v="]');if(m&&m.src.split("?v=")[1])v=m.src.split("?v=")[1];}catch(e){}var out={version:v,at:new Date().toISOString(),unapplied:[]};document.querySelectorAll(".v41-unapilled, .v41-unapplied").forEach(function(cb){if(cb.checked)out.unapplied.push({index:Number(cb.dataset.i)+1,title:V41_CHANGES[Number(cb.dataset.i)][0]});});out.appliedCount=V41_CHANGES.filter(function(c){return c[1]==="applied";}).length;out.pendingCount=V41_CHANGES.filter(function(c){return c[1]==="pending";}).length;return out;}
  ...
  var buildLocal="نامشخص";try{var m=document.querySelector('script[src*="crm-features-v20.js?v="]');buildLocal=m?String(m.src.split("?v=")[1]):"?";}catch(e){}
  ...
  function loadedBuildVersion(){try{var m=document.querySelector('script[src*="crm-features-v20.js?v="]');return m?String(m.src.split("?v=")[1]):"";}catch(e){return "";}}
  ...
  function restoreDomFieldOrder(){var explicit={};try{explicit=JSON.parse(localStorage.getItem(MANAGER_ORDER_KEY)||"{}");}catch(e){}domOrderBusy=true;lockableGrids().forEach(function(grid,i){var seq=explicit[gridLockKey(grid,i)];if(!Array.isArray(seq)||!seq.length)return;var groups={};Array.prototype.forEach.call(grid.children,function(g){var id=groupAnchor(g);if(id)groups[id]=g;});
    // v11.61.0: فقط در صورت تفاوت واقعی با ترتیب ذخیره‌شده جابه‌جا کن؛ بازنویسیِ همان ترتیب، حلقه خودتغذی MutationObserver می‌ساخت
    var currentIds=Array.prototype.map.call(grid.children,function(g){return groupAnchor(g);}).filter(Boolean);
