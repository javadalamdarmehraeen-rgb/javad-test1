...
(function () {
  ...
  function bindPlacedPharmacyNoticeGuard(){var inp=$("orderPharmacyName"),alertBox=$("existingPharmacyTopAlert"),pickBox=$("orderPharmacyPickBox");if(!inp||inp.dataset.placedGuard)return;inp.dataset.placedGuard="1";function enforce(){var placed=inp.dataset.v20PlacedName&&norm(inp.value)===norm(inp.dataset.v20PlacedName)&&String(($("orderPharmacyMatchedId")||{}).value||"");if(placed){if(alertBox&&(alertBox.style.getPropertyValue("display")!=="none"||alertBox.style.getPropertyPriority("display")!=="important"))alertBox.style.setProperty("display","none","important");if(pickBox){if(!pickBox.hidden)pickBox.hidden=true;if(pickBox.style.getPropertyValue("display")!=="none"||pickBox.style.getPropertyPriority("display")!=="important")pickBox.style.setProperty("display","none","important");}}}inp.addEventListener("input",function(){if(inp.dataset.v20PlacedName&&norm(inp.value)!==norm(inp.dataset.v20PlacedName))delete inp.dataset.v20PlacedName;setTimeout(enforce,0);},true);if(window.MutationObserver){if(alertBox)new MutationObserver(enforce).observe(alertBox,{attributes:true,childList:true,subtree:true});if(pickBox)new MutationObserver(enforce).observe(pickBox,{attributes:true,childList:true,subtree:true});}enforce();}
  ...
  function runtimeVersion(){
    ...
    return m?decodeURIComponent(m[1]):"11.35.0";
  }
  ...
  function privacyList(list,permissionKey){var S=st(),u=v20CurrentUser(),manager=v20IsManager(),perms=(u&&u.permissions)||{},active={};((S&&S.users)||[]).forEach(function(x){if(x&&x.fullName)active[norm(x.fullName)]=1;});var clean=(list||[]).filter(function(r){var n=String(r&&r.repName||r&&r.name||"");return!n||/مدیر سیستم/.test(n)||active[norm(n)];});if(manager||perms[permissionKey]===true)return clean.slice();return clean.filter(function(r){return u&&((r.repId&&String(r.repId)===String(u.id))||String(r.repName||r.name||"")===String(u.fullName||""));});}
  function bindPrivacyRenderers(){if(document.body.dataset.privacyRenderers)return;document.body.dataset.privacyRenderers="1";[["renderActivityLogTable","activityLog","activity_all_reps"],["renderLeavesTable","leaves","hr_all_leaves"],["renderRepRoutesTable","repRoutes","fld_all_routes"],["renderRepHomesTable","repHomes","fld_all_homes"],["renderSalesTargetsTable","salesTargets","target_all_reps"]].forEach(function(cfg){var old=window[cfg[0]];if(typeof old!=="function")return;window[cfg[0]]=function(){var S=st(),all=S[cfg[1]],filtered=privacyList(all,cfg[2]);S[cfg[1]]=filtered;try{return old.apply(this,arguments);}finally{S[cfg[1]]=all;}};});var activityMap=window.renderActivityMapAndChart;if(typeof activityMap==="function")window.renderActivityMapAndChart=function(){var S=st(),all=S.activityLog;S.activityLog=privacyList(all,"activity_all_reps");try{return activityMap.apply(this,arguments);}finally{S.activityLog=all;}};var monthly=window.renderMonthlyReportsTable;if(typeof monthly==="function")window.renderMonthlyReportsTable=function(){var S=st(),reps=S.reps,visits=S.visits,orders=S.orders;S.reps=privacyList(reps,"rep_all_reports");S.visits=privacyList(visits,"rep_all_reports");S.orders=privacyList(orders,"rep_all_reports");try{return monthly.apply(this,arguments);}finally{S.reps=reps;S.visits=visits;S.orders=orders;}};}
  ...
  function init() {
    ...
    try { bindTargetsV20(); setupTargetPlannerV34(); } catch (e) {}
