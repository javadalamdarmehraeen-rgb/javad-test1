...
نسخه سورس 12.14.0 (نوبت ۱۲۸): سه نقص واقعیِ نسخه روی گیت‌هاب رفع شد — نسخه در ۹ فایل
یکسان شد، `public/index.php` ساخته و رسمی شد، و انتظارات کهنهٔ تست‌ها به‌روز شد
(نتیجه ۱۳۸/۱۳۹). سربرگ کنار لوگو دقیقاً سه خط شد: نام برنامه و شرکت /
`نسخه 12.14.0` با ارقام لاتین / `طنین طب طاها  TANIN TEB TAHA`، و با MutationObserver
در برابر رنگ‌آمیزی‌های قدیمی قفل می‌شود. برنامه حالا مستقل از VPN بالا می‌آید:
همه درخواست‌های بین‌دامنه‌ای پس‌زمینه با تایم‌اوت ۶ ثانیه، نقشه بدون کاشی هم کار
می‌کند و ژئوکد سرور تایم‌اوت دارد. دامنه سوم نت‌افراز `ndcohub.com` به برنامه افزوده
شد و همگام بین هر سه دامنه (رندر + دو نت‌افراز) هم در مرورگر و هم در
`api.php?target=all` برقرار است؛ `ndcohub.ir` به‌خاطر گواهی نامعتبر از چرخه خارج است.
