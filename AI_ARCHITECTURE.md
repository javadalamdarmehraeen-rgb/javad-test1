...
## معماری تحویل نوبت ۱۲۸ — سه دامنه، بدون VPN، سربرگ سه‌خطی (2026-08-31)

**۱) سه دامنهٔ فعال و جریان همگام**

```text
گوشی/ویندوز ──► origin (نت‌افراز: api.php | رندر: server.js)  ← منبع حقیقت همان دستگاه
                     │
        (پس‌زمینه، ۶ ثانیه تایم‌اوت، بدون بلاک)
                     ├──► https://javad-test1.onrender.com/api/state
                     ├──► https://mehraeinpharma.ir/api/state
                     └──► https://ndcohub.com/api/state
                     کشیدن → ادغام شناسه‌محور (جدیدتر برنده، بدون کاهش رکورد) → فرستادن ادغام
```

- نت‌افراز علاوه بر مرورگر، مسیر سروری `api.php?path=sync&target=all` دارد
  (`peer_hosts()` + `merge_state()` + `sync_all_peers()`) تا حتی وقتی گوشی به رندر
  دسترسی ندارد، داده از نت‌افرازِ دیگر بیاید.
- CORS به‌صورت پویا (`SetEnvIf Origin`) برای هر سه دامنه پاسخ می‌دهد؛ هدر ثابتِ
  تک‌دامنه حذف شد چون پاسخ PHP را بازنویسی می‌کرد.
- `ndcohub.ir` (گواهی نامعتبر) از `peerList` حذف است اما در فهرست CORS باقی مانده.

**۲) استقلال از VPN**

- لایه `v12.15.0` در `crm-bundle.js` تمام درخواست‌های بین‌دامنه‌ای را از
  `softFetch` با `AbortController` و ۶ ثانیه تایم‌اوت عبور می‌دهد؛
  در `navigator.onLine === false` اصلاً درخواستی فرستاده نمی‌شود.
- تایم‌اوت‌های ۲۰ ثانیه‌ایِ قدیمی (v12.01/v12.02) به ۶ ثانیه کاهش یافت.
- `fetch` پیش از نصب لایه یک‌بار گرفته می‌شود تا بازگشت بی‌پایان رخ ندهد.
- کاشی نقشه: خطاها شمارش و منبع بعدی امتحان می‌شود؛ اگر هیچ‌کدام نرسد،
  یک پیام غیرمسدودکننده روی نقشه می‌نشیند و نقشه فعال می‌ماند.
- ژئوکد/معکوس در `server.js` حالا ۶ ثانیه تایم‌اوت دارد (پیش از این بی‌انتها بود).

**۳) سربرگ و نشان نسخه**

- سه المان: `#headerCompanyNameDisplay` (نام برنامه و شرکت)، `#crmBuildBadge`
  (`نسخه 12.15.0` با ارقام لاتین)، `#headerBrandLine` (برند لاتین/فارسی).
- `MutationObserver` روی `.logo-text` هر بازنویسیِ قدیمی را بلافاصله اصلاح می‌کند.
- شماره نسخه همیشه از `window.CRM_APP_VERSION` است (قانون «نشان نسخه واحد»).

**۴) `public/index.php`**

ورودی ریشهٔ نت‌افراز: `index.html` را مستقیماً سرو می‌کند (بدون ریدایرکت/حلقه)،
در نبود آن به `login.html` می‌رود. خروجی `npm run build-static` آن را همراه سایر
فایل‌ها کپی می‌کند.
