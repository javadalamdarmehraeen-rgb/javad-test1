/**
 * اجرای واقعیِ لایهٔ v12.18.1 — قانون ۹۲: «هیچ بندی بدون مدرک تأییدشده نیست».
...
assert.ok(from > 0, 'لایهٔ v12.18.1 در انتهایِ باندل پیدا نشد');
...
test('v12.18.1: قانونِ ارقام — toLocaleStringِ فارسی عددِ لاتین می‌دهد و تاریخ شمسی می‌ماند', () => {
  const env = bootLayer();
...
/* ───────── ۱) ریشه‌پاک‌کن ───────── */
test('v12.18.1: ریشه‌پاک‌کن کلیدهایِ کهنه را می‌زداید و state/ورود/سولو/پروژهٔ sync را نگه می‌دارد', () => {
  ...
  assert.equal(ls.getItem('CRM_V1218_PURGED'), '12.18.1', 'مُهرِ یک‌بارمصرف');
  env.flush();
  assert.equal(env.win.__reloads, 1, 'یک بار تازه‌سازی پس از پاک‌سازی');
});

test('v12.18.1: پاک‌سازی در نسخهٔ همین‌بار اجراشده تکرار نمی‌شود', () => {
  const ls = storageStub({ CRM_V1218_PURGED: '12.18.1', CRM_DIAG_LOG: 'keep?no' });
...
test('v12.18.1: شماره‌گذاریِ یکتا روی متادیتا + فیلدِ سفارشی با هم (رفعِ بازگشتِ ترتیب پس از رفرش)', () => {
  const state = {
...
test('v12.18.1: applySavedLayoutV82 و applyFullFormLayout با بی‌تغیر بودن، DOM را نمی‌لمسانند (ضدِ پرش)', () => {
  let calls = 0;
...
test('v12.18.1: مثبتِ بند ۴ — تغییرِ ترتیب، فوراً روی تب اصلی اعمال می‌شود و دوباره‌نویسیِ بی‌تغیر انجام نمی‌شود', () => {
...
test('v12.18.1+: purgeNow مُهرِ «انجام‌شده» را برمی‌دارد، همه‌چیز را دوباره جارو می‌کند، state را نگه می‌دارد و یک‌بار reload می‌کند', () => {
  const ls = storageStub({
    CRM_V1218_PURGED: '12.18.1',
  ...
  assert.equal(ls._map.get('CRM_V1218_PURGED'), '12.18.1', 'مُهرِ نسخه در همانِ لحظه تازه شد');
...
/* ───────── ۳) بند ۹: تردد ───────── */
test('v12.18.1: «نمایش تردد» بدون alert رسم می‌کند؛ نقشهٔsvg و fitBounds صدا زده می‌شوند', () => {
...
/* ───────── ۴) بند ۱۲: ساعت ───────── */
test('v12.18.1: گزینهٔ ساعت باز‌درج می‌شود و ثانیه از ورودی‌ها بریده می‌شود', () => {
  const env = bootLayer({});
...
test('v12.18.1: ماتریسِ ریز، ۳۱ تب یا بیشتر + ستونِ کلیدِ دسترسی + نوشتنِ *_access روی کاربر', () => {
  const state = {
...
test('v12.18.1: آلارمِ ویزیت با فرمتِ مرکزِ اعلان‌ها به نماینده+سرپرست+مدیر می‌رسد (یک‌بار، بدونِ تکرار)', () => {
...
test('v12.18.1: GETهایِ state/sync از مسیرِ اصیل تا ۲۴۰ ثانیه فقط یک‌بار می‌روند', async () => {
  let real = 0;
...
/* ───────── ۸) صلحِ v68↔v73 ───────── */
test('v12.18.1: شبح‌هایِ میزبانِ عملیات ساخته و از حذف محافظت می‌شوند', () => {
  const env = bootLayer({});
...
test('v12.18.1: بنرِ لاتین، نسخه در همه‌ی سطح‌ها و فایل‌ها، و ریشه‌پاک‌کنِ سرور', () => {
  const env = bootLayer({});
  ...
  assert.equal(badge.textContent, 'نسخه 12.18.1', 'بنرِ ارقامِ لاتین');
  const pairs = [
    ['package.json', /"version":\s*"12\.18\.1"/],
    ['server.js', /const APP_VERSION = "12\.18\.1"/],
    ['public/crm-app.js', /CRM_APP_VERSION = "12\.18\.1"/],
    ['public/index.html', /BUILD="12\.18\.1"/],
    ['public/sw.js', /BUILD = "12\.18\.1"/],
    ['public/api.php', /CRM_APP_VERSION", "12\.18\.1"/],
    ['public/login.html', /نسخه 12\.18\.1/],
    ['README.md', /نسخه‌ی جاریِ این ریپو \(GitHub main\): \*\*12\.18\.1\*\*/]
  ];
  for (const [f, re] of pairs) {
    const t = readFileSync(new URL(f, root), 'utf8');
    assert.ok(re.test(t), 'نسخه در ' + f + ' باید 12.18.1 باشد');
  }
