...
test('v12.18.3: ریشه‌پاک‌کن کلیدهایِ کهنه را می‌زداید و state/ورود/سولو/پروژهٔ sync را نگه می‌دارد', () => {
  ...
  assert.equal(ls.getItem('CRM_V1218_PURGED'), '12.18.4', 'مُهرِ یک‌بارمصرف');
  env.flush();
...
test('v12.18.3: پاک‌سازی در نسخهٔ همین‌بار اجراشده تکرار نمی‌شود', () => {
  const ls = storageStub({ CRM_V1218_PURGED: '12.18.4', CRM_DIAG_LOG: 'keep?no' });
...
test('v12.18.3+: purgeNow مُهرِ «انجام‌شده» را برمی‌دارد، همه‌چیز را دوباره جارو می‌کند، state را نگه می‌دارد و یک‌بار reload می‌کند', () => {
  ...
  assert.equal(ls._map.get('CRM_V1218_PURGED'), '12.18.4', 'مُهرِ نسخه در همانِ لحظه تازه شد');
...
test('v12.18.3: بنرِ لاتین، نسخه در همه‌ی سطح‌ها و فایل‌ها، و ریشه‌پاک‌کنِ سرور', () => {
  ...
  assert.equal(badge.textContent, 'نسخه 12.18.4', 'بنرِ ارقامِ لاتین');
  const pairs = [
    ['package.json', /"version":\s*"12\.18\.4"/],
    ['server.js', /const APP_VERSION = "12\.18\.4"/],
    ['public/crm-app.js', /CRM_APP_VERSION = "12\.18\.4"/],
    ['public/index.html', /BUILD="12\.18\.4"/],
    ['public/sw.js', /BUILD = "12\.18\.4"/],
    ['public/api.php', /CRM_APP_VERSION", "12\.18\.4"/],
    ['public/login.html', /نسخه 12\.18\.4/],
    ['README.md', /نسخه‌ی جاریِ این ریپو \(GitHub main\): \*\*12\.18\.4\*\*/]
  ];
  for (const [f, re] of pairs) {
    const t = readFileSync(new URL(f, root), 'utf8');
    assert.ok(re.test(t), 'نسخه در ' + f + ' باید 12.18.4 باشد');
  }
