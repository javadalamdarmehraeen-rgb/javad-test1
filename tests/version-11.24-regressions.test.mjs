...
test('next-chat GitHub handoff records exact publish truth and engine map', () => {
  assert.match(githubHandoff,/نسخه آماده سورس:[\s\S]*12\.17.1/);
...
test('v38 cache rescue automatically forces a fresh build without deleting CRM data', () => {
  ...
  assert.match(server,/"X-CRM-Build": APP_VERSION/);assert.match(server,/const APP_VERSION = "12\.17.1"/);
  ...
  assert.match(app,/CRM_BUILD_ACTIVE/);assert.match(app,/register\('\/sw\.js\?v=12\.17.1'/);
});
...
test('PWA activation is automatic and diagnostics never request manual refresh', () => {
  assert.match(app,/register\('\/sw\.js\?v=12\.17.1', \{ scope: '\/', updateViaCache: 'none' \}\)/);
...
test('GPS quality rule remains stable and full address parts stay Iran-first without postcode', () => {
  /* v12.14: همان هدفِ دقت (۱۰ متر) اما با خروجِ تطبیقی — دیگر ۱۵ ثانیه معطل نمی‌مانیم */
  assert.match(v9, /cur\.accuracy <= 10\) finish\(cur\)/);
  assert.match(v9, /waited >= 2500 && cur\.accuracy <= 30\) finish\(cur\)/);
  assert.match(v9, /addressPromise = /);
  assert.match(v9, /}, 6000\)/);
  assert.match(v9, /enableHighAccuracy: true, timeout: 8000, maximumAge: 60000/);
  assert.doesNotMatch(v9, /timeout:15000,maximumAge:0/);
...
test('v11.61.0 safe cache-hardening', () => {
  ...
  assert.match(app,/CRM_APP_VERSION = "12.17.1"/);
...
test('v11.61.0 mutation-loop guards: idempotent reorder/restore, structural position-lock, quiet permissions', () => {
  ...
  const v20file = read('../tests/fixtures/crm-features-v20.js');
...
test('v11.72.0 durable target ops + mm gap + row number', () => {
  ...
  const css = read('../public/style.css');
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
...
test('v11.72.0 server-first paint + no-spin qty + per-row ops + dist achieved/remain', () => {
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(app, /register\('\/sw\.js\?v=12\.17.1'/);
...
test('v11.73.0 live state bind + delayed unveil + durable dist save + kill old ops hosts', () => {
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(sw, /const BUILD = "12.17.1"/);
  ...
  assert.match(html, /var BUILD="12.17.1"/);
  ...
  assert.match(login, /var BUILD="12.17.1"/);
...
test('v11.93.0 static netafraz + env hubs + sync-all + timeout hub', () => {
  ...
  assert.match(html, /crm-hub\.js\?v=12\.17.1/);
  assert.match(html, /crm-runtime\.js\?v=12\.17.1/);
...
test('v11.94.0 netafraz php api independent + leaflet root + no api 404 fake', () => {
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(html, /leaflet.css\?v=12\.17.1/);
  assert.match(html, /leaflet.js\?v=12\.17.1/);
...
test('v11.95.0 same version badge + origin-only hubs + taha company name', () => {
  ...
  assert.match(data, /companyName: "برنامه ویزیت و گزارشات \(مهر آیین نیک دارو\)"/);
  assert.match(html, /نسخه 12\.17.1/);
  assert.match(html, /id="headerCompanyNameDisplay">برنامه ویزیت و گزارشات \(مهر آیین نیک دارو\)/);
  assert.match(login, /نسخه 12\.17.1/);
...
test('v11.96.0 netafraz api/sync to render + gen stamp + no empty wipe', () => {
  ...
  assert.match(html, /crm-app\.js\?v=12\.17.1/);
});
...
test('v11.98.0 sw always returns Response + netafraz no Options 500 + valid api-config', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const sw = read('../public/sw.js');
  const php = read('../public/api.php');
  ...
  const html = read('../public/index.html');
  const login = read('../public/login.html');
  const cfg = read('../public/api-config.php');
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(sw, /const BUILD = "12.17.1"/);
  assert.match(sw, /function fallbackResponse/);
  assert.match(sw, /asResponse/);
  assert.match(sw, /fetchWithTimeout/);
  assert.match(sw, /cache: "reload"/);
  assert.doesNotMatch(ht, /Options -Indexes/);
  assert.doesNotMatch(st, /Options -Indexes/);
  assert.match(st, /array\(/);
  assert.match(st, /api-config.json/);
  assert.match(php, /api-config.json/);
  assert.match(php, /strpos\(\$raw, "array\("/);
  assert.match(cfg, /array\(/);
  assert.doesNotMatch(cfg, /return \{/);
  assert.ok(b.includes("v11.98.0"));
  assert.match(b, /window.v98BootFix/);
  assert.match(html, /nosw/);
  assert.match(login, /nosw/);
  assert.match(html, /نسخه 12\.17.1/);
});
test('v11.99.0 independent fast netafraz + background peer sync + ignore old files', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const php = read('../public/api.php');
  const hub = read('../public/crm-hub.js');
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(php, /crm-live-data.json/);
  assert.match(php, /crm-netafraz-data.json/);
  assert.match(php, /queued/);
  assert.match(php, /fastcgi_finish_request/);
  assert.doesNotMatch(php.split('strpos($p, "state")')[1].split('strpos($p, "bulk")')[0], /pull_render\(\)/);
  assert.match(hub, /v99FastLocal/);
  assert.match(hub, /12000/);
  assert.match(hub, /function retry/);
  assert.match(app, /v99DropOldProgramFiles/);
  assert.match(sw, /if \(request\.mode === "navigate"\) return;/);
  assert.ok(b.includes("v11.99.0"));
  assert.match(b, /window.v99FastIndependent/);
  assert.match(b, /boot never waits on Render/);
});
test('v12.00.0 netafraz index.php no 403 + same badge + taha company', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const ht = read('../public/.htaccess');
  const ip = read('../public/index.php');
  const st = read('../build-static.js');
  const b = read('../public/crm-bundle.js');
  const html = read('../public/index.html');
  const login = read('../public/login.html');
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(app, /window.CRM_APP_VERSION = CRM_APP_VERSION/);
  assert.match(app, /window.v12TahaName/);
  assert.match(app, /v12CanonicalCompany/);
  assert.match(ht, /DirectoryIndex index\.php/);
  assert.match(ht, /RewriteBase \//);
  assert.doesNotMatch(ht, /RewriteRule \^panel\/\?\$ \/index/);
  assert.match(ip, /login\.html/);
  assert.match(st, /RewriteBase/);
  assert.match(st, /DirectoryIndex index\.php/);
  assert.ok(b.includes("v12.00.0"));
  assert.match(b, /window.v12SameBadge/);
  assert.match(html, /window.CRM_APP_VERSION=BUILD/);
  assert.match(login, /window.CRM_APP_VERSION=BUILD/);
  assert.match(html, /نسخه 12\.17.1/);
});
test('v12.01.0 no ndcohub cert fetch + osm tiles on static + pull then push', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const php = read('../public/api.php');
  const hub = read('../public/crm-hub.js');
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(hub, /ndcohub/);
  assert.match(app, /tile\.openstreetmap\.de/);
  assert.match(app, /\/api\/tiles\/\{z\}\/\{x\}\/\{y\}\.png/);
  assert.match(app, /window.crmAddMapTiles/);
  assert.ok(b.includes("v12.01.0"));
  assert.match(b, /window.v1201CertTilesSync/);
  assert.match(php, /filled from render/);
  assert.match(php, /strpos\(\$p, "tiles\/"\)/);
  const rh = srv.split('function runtimeHubs')[1].split('function isCrmHubHost')[0];
  assert.doesNotMatch(rh, /ndcohub\.ir/);
  assert.match(srv, /ndcohub\.ir/);
});
test('v12.02.0 auto pull render only + ops restore + compact mobile', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const hub = read('../public/crm-hub.js');
  const b = read('../public/crm-bundle.js');
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(app, /v12TakeRegisteredOnly/);
  assert.match(app, /window.v12OpsOnlyRestore/);
  /* v12.12: سه دامنه با هم همگام می‌شوند */
  assert.match(hub, /CRM_HUB_DOMAINS/);
  assert.match(hub, /https:\/\/ndcohub\.com/);
  assert.match(hub, /https:\/\/mehraeinpharma\.ir/);
  assert.match(b, /window.v1202AutoSync/);
  assert.match(css, /v12\.02\.0: موبایل فشرده/);
  assert.match(ht, /Access-Control-Allow-Origin/);
  assert.match(ht, /REQUEST_METHOD\} OPTIONS/);
});
test('v12.03.0 upload does not wipe pharmacies + skip live json in static', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const st = read('../build-static.js');
  const b = read('../public/crm-bundle.js');
  ...
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(app, /v99DropOldProgramFiles/);
  assert.match(app, /هرگز داروخانه\/پزشک\/سفارش را با آپلود نسخه خالی نکن/);
  assert.match(st, /crm-live-data\.json/);
  assert.match(st, /KHANAN-APLOAD/);
  assert.ok(b.includes("v12.03.0"));
  assert.match(b, /window.v1203KeepData/);
});
test('v12.04.0 data folder live json auto-create not in js root', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const php = read('../public/api.php');
  const st = read('../build-static.js');
  ...
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(php, /crm-live-data.json/);
  assert.match(php, /\$DATA_DIR/);
  assert.match(php, /function ensure_dir/);
  assert.match(st, /path.join\(DEST, "data"\)/);
  assert.match(st, /KHANAN-APLOAD/);
});
test('v12.05.0 flat static-build files only + fill empty from render', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const php = read('../public/api.php');
  const st = read('../build-static.js');
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(st, /function copyFlat/);
  assert.match(st, /crm-netafraz-data.json/);
  assert.match(php, /function fill_if_empty/);
  assert.match(php, /crm-live-data.json/);
  const stateBlk = php.split('strpos($p, "state")')[1].split('strpos($p, "bulk")')[0];
  assert.doesNotMatch(stateBlk, /pull_render\(\)/);
  assert.match(stateBlk, /fill_if_empty/);
});
test('v12.06.0 device sync netafraz origin api.php path state', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const hub = read('../public/crm-hub.js');
  const b = read('../public/crm-bundle.js');
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(hub, /function altApi/);
  assert.match(hub, /api\.php\?path=/);
  assert.match(b, /window.v1206DeviceSync/);
  assert.match(php, /function fill_if_empty/);
  assert.match(hub, /function retry/);
  assert.match(hub, /12000/);
});
test('v12.07.0 form header pricing routes homes persist', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const b = read('../public/crm-bundle.js');
  const css = read('../public/style.css');
  ...
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(b, /window.v1207FormFix/);
  assert.match(b, /roundNice/);
  assert.match(css, /v12\.07\.0: هدر سبز/);
  assert.match(html, /btn-menu-hamburger/);
});
test('v12.08.0 persist no wipe + one specialty field + no geo loop', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const b = read('../public/crm-bundle.js');
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(b, /window.v1208PersistFix/);
  assert.match(b, /window.v1207FormFix/);
  assert.match(b, /v12-spec-combo/);
  assert.match(b, /recN\(st\)>0 && recN\(d\)<recN\(st\)/);
});
test('v12.17.1 layout sync + consVat edit + specialty combo', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const b = read('../public/crm-bundle.js');
  assert.match(app, /CRM_APP_VERSION = "12.17.1"/);
  assert.match(srv, /const APP_VERSION = "12.17.1"/);
  assert.match(app, /formFieldMeta/);
  assert.match(b, /data-k='consVat'/);
  assert.match(b, /getCurrentPositionSafe/);
  assert.match(b, /v12-spec-combo/);
});

/* =========================================================================
   v12.17.1 — تحویل نوبت ۱۲۸: رفع سه نقص + سربرگ سه‌خطی + بدون VPN + سه دامنه
   ========================================================================= */
test('v12.17.1 one version everywhere (7 version files in sync)', () => {
  const pkg = read('../package.json');
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const sw = read('../public/sw.js');
  ...
  const hub = read('../public/crm-hub.js');
  const php = read('../public/api.php');
  assert.match(pkg, /"version": "12\.17.1"/);
  assert.match(app, /const CRM_APP_VERSION = "12\.17.1"/);
  assert.match(srv, /const APP_VERSION = "12\.17.1"/);
  assert.match(sw, /const BUILD = "12\.17.1"/);
  assert.match(html, /var BUILD="12\.17.1"/);
  assert.match(login, /var BUILD="12\.17.1"/);
  assert.doesNotMatch(hub, /12\.09\.[0-9]/);
  assert.doesNotMatch(php, /12\.09\.[0-9]/);
  /* هیچ نشانی از نسخه قبلی در دارایی‌ها نمانده باشد */
  assert.doesNotMatch(html, /\?v=12\.09\.[0-9]/);
  assert.doesNotMatch(login, /\?v=12\.09\.[0-9]/);
});

test('v12.17.1 defect #2: public/index.php exists, serves index.html, is official', () => {
  const ip = read('../public/index.php');
  const list = read('../OFFICIAL_FILELIST.txt');
  const st = read('../build-static.js');
  assert.match(ip, /login\.html/);            /* fallback */
  assert.match(ip, /index\.html/);            /* صفحه اصلی */
  assert.match(ip, /readfile/);               /* بدون ریدایرکت حلقه‌ای */
  assert.match(ip, /12\.17.1/);
  assert.match(list, /^public\/index\.php$/m);
  assert.match(st, /copyFlat/);               /* index.php همراه فایل‌ها کپی می‌شود */
});

test('v12.17.1 header is exactly three lines next to the logo', () => {
  const html = read('../public/index.html');
  const b = read('../public/crm-bundle.js');
  const data = read('../public/crm-data.js');
  const app = read('../public/crm-app.js');
  /* خط ۱ — نام برنامه و شرکت */
  assert.match(html, /id="headerCompanyNameDisplay">برنامه ویزیت و گزارشات \(مهر آیین نیک دارو\)</);
  assert.match(data, /companyName: "برنامه ویزیت و گزارشات \(مهر آیین نیک دارو\)"/);
  /* خط ۲ — نسخه با ارقام لاتین */
  assert.match(html, /id="crmBuildBadge"[^>]*>نسخه 12\.17.1</);
  assert.match(b, /نسخه " \+ ver\(\)/);
  assert.match(b, /setTxt\("crmBuildBadge", "نسخه " \+ ver\(\)\)/);
  /* خط ۳ — برند */
  assert.match(html, /id="headerBrandLine">طنین طب طاها&nbsp;&nbsp;TANIN TEB TAHA</);
  assert.match(b, /setTxt\("headerBrandLine", BRAND\)/);
  assert.match(b, /طنین طب طاها  TANIN TEB TAHA/);
  /* پایداری در برابر رنگ‌آمیزی‌های قدیمی */
  assert.match(b, /MutationObserver/);
  assert.match(b, /window\.v1212Header/);
  assert.match(app, /مهر آیین نیک دارو/);
});

test('v12.17.1 app boots and works with VPN on or off', () => {
  const b = read('../public/crm-bundle.js');
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const sw = read('../public/sw.js');
  /* ۱) هیچ درخواست بین‌دامنه‌ای برنامه را معطل نمی‌گذارد */
  assert.match(b, /window\.v1212NoVpn/);
  assert.match(b, /var CROSS_MS = 6000/);
  assert.match(b, /softFetch\(url, opts, CROSS_MS\)/);
  assert.match(b, /navigator\.onLine === false/);
  assert.match(b, /boot never waits on Render/);          /* نشانگر قدیمی هم حفظ شده */
  /* ۲) دیگر هیچ تایم‌اوت ۲۰ ثانیه‌ای برای هم‌دامنه‌ها نیست */
  assert.doesNotMatch(b, /ctrl\.abort\(\); \}\} catch \(eA\) \{\} \}, 20000\)/);
  /* ۳) نقشه بدون کاشی هم کار می‌کند */
  assert.match(app, /crm-tile-notice/);
  assert.match(app, /تصویر نقشه در دسترس نیست/);
  assert.match(app, /map\.__crmTileOk/);
  /* ۴) ژئوکد/معکوس در سرور تایم‌اوت دارد (v12.14: مسابقهٔ موازی، ۴ ثانیه) */
  assert.match(srv, /function geoRace\(/);
  assert.match(srv, /geoRace\(urls, 4000, isRev\)/);
  assert.match(srv, /const ac = new AbortController\(\)/);
  /* ۵) سرویس‌ورکر مانع بالا آمدن نمی‌شود */
  assert.match(sw, /const BUILD = "12\.17.1"/);
});

test('v12.17.1 three domains (Render + 2 Netafraz) sync on phone and Windows', () => {
  const hub = read('../public/crm-hub.js');
  const b = read('../public/crm-bundle.js');
  const php = read('../public/api.php');
  const srv = read('../server.js');
  const st = read('../build-static.js');
  const cfg = read('../public/api-config.json');
  const phpCfg = read('../public/api-config.php');
  const ht = read('../public/.htaccess');
  const yaml = read('../render.yaml');
  const app = read('../public/crm-app.js');

  /* فهرست سه دامنه در لایه مرورگر */
  assert.match(hub, /https:\/\/javad-test1\.onrender\.com/);
  assert.match(hub, /https:\/\/mehraeinpharma\.ir/);
  assert.match(hub, /https:\/\/ndcohub\.com/);
  assert.match(hub, /window\.CRM_HUB_DOMAINS/);
  assert.match(b, /window\.v1212ThreeHubs/);
  assert.match(b, /https:\/\/ndcohub\.com/);

  /* سمت PHP نت‌افراز */
  assert.match(php, /https:\/\/ndcohub\.com/);
  assert.match(php, /function peer_hosts\(\)/);
  assert.match(php, /function merge_state\(/);
  assert.match(php, /function sync_all_peers\(/);
  assert.match(php, /\$target === "all"/);

  /* سمت سرور رندر */
  assert.ok(srv.indexOf('ndcohub\\.com|ndcohub\\.ir|mehraeinpharma\\.ir') >= 0, 'هاست‌های مجاز سرور');
  assert.match(srv, /https:\/\/mehraeinpharma\.ir", "https:\/\/ndcohub\.com/);
  assert.match(srv, /https:\/\/ndcohub\.com/);

  /* خروجی استاتیک و تنظیمات */
  assert.match(st, /https:\/\/ndcohub\.com/);
  assert.match(cfg, /https:\/\/ndcohub\.com/);
  assert.match(phpCfg, /https:\/\/ndcohub\.com/);
  assert.ok(ht.indexOf('ndcohub\\.com') >= 0, 'CORS نت‌افراز: ndcohub.com');
  assert.ok(ht.indexOf('mehraeinpharma\\.ir') >= 0, 'CORS نت‌افراز: mehraeinpharma.ir');
  assert.match(yaml, /https:\/\/ndcohub\.com/);

  /* برنامه دامنه جدید را به‌عنوان هاست ایرانی می‌شناسد */
  assert.ok(app.indexOf('ndcohub\\.(ir|com)') >= 0, 'برنامه دامنه ndcohub.com را ایرانی می‌داند');
  assert.ok(b.indexOf('ndcohub\\.(ir|com)') >= 0, 'باندل دامنه ndcohub.com را ایرانی می‌داند');
  assert.ok(app.indexOf('ndcohub\\.(ir|com)') >= 0, 'برنامه دامنه ndcohub.com را ایرانی می‌داند');
  assert.ok(b.indexOf('ndcohub\\.(ir|com)') >= 0, 'باندل دامنه ndcohub.com را ایرانی می‌داند');
});
