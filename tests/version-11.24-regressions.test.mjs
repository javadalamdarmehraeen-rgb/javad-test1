...
test('next-chat GitHub handoff records exact publish truth and engine map', () => {
  assert.match(githubHandoff,/نسخه آماده سورس:[\s\S]*11\.43\.1/);
...
test('new build clears only old asset caches before revealing app and prevents manager-screen flash', () => {
  assert.match(html,/var BUILD="11\.43\.1",key="CRM_ASSET_BUILD"/);
...
test('v38 cache rescue automatically forces a fresh build without deleting CRM data', () => {
  ...
  assert.match(server,/"X-CRM-Build": APP_VERSION/);assert.match(server,/const APP_VERSION = "11\.43\.1"/);
  ...
  assert.match(app,/CRM_BUILD_ACTIVE/);assert.match(app,/register\('\/sw\.js\?v=11\.43\.1'/);
});
...
test('PWA activation is automatic and diagnostics never request manual refresh', () => {
  assert.match(app,/register\('\/sw\.js\?v=11\.43\.1', \{ scope: '\/', updateViaCache: 'none' \}\)/);
