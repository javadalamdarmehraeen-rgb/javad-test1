...
test('v11.95.0 same version badge + origin-only hubs + taha company name', () => {
  ...
  assert.match(html, /نسخه ۱۲\.۱۲\.۰/);
  assert.match(html, /id="headerCompanyNameDisplay">طنین طب طاها/);
  assert.match(login, /نسخه ۱۲\.۱۲\.۰/);
...
test('v12.02.0 auto pull render only + ops restore + compact mobile', () => {
  ...
  assert.match(hub, /Render never fetches Netafraz/);
...
test('v12.09.0 layout sync + consVat edit + specialty combo', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const b = read('../public/crm-bundle.js');
  assert.match(app, /CRM_APP_VERSION = "12.12.0"/);
...
test('v12.10.0 live sync + home gps plate + geo freeze + consVat remainder', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const b = read('../public/crm-bundle.js');
  const php = read('../public/api.php');
  ...
  assert.match(app, /CRM_APP_VERSION = "12.12.0"/);
  assert.match(srv, /const APP_VERSION = "12.12.0"/);
  assert.match(b, /window.v1210LiveSync/);
  assert.match(b, /window.v1210HomeGps/);
  assert.match(b, /cv\*\(1-vat\/100\)/);
  assert.match(php, /_lastSavedAt/);
  assert.match(app, /hm\.plate/);
  assert.match(html, /<th>پلاک<\/th>/);
  assert.match(b, /هنگام pull پوش نکن/);
});

test('v12.11.0 home plate floor + geo skip global + live remainder + autosync', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const b = read('../public/crm-bundle.js');
  const html = read('../public/index.html');
  assert.match(app, /CRM_APP_VERSION = "12.12.0"/);
  assert.match(srv, /const APP_VERSION = "12.12.0"/);
  assert.match(b, /window.v1211Fix/);
  assert.match(b, /v1211SkipGeo/);
  assert.match(b, /esc\(h\.plate/);
  assert.match(b, /cv\*\(1-vat\/100\)/);
  assert.match(html, /<th>پلاک<\/th>/);
  assert.match(html, /btnSaveRepHome/);
  assert.match(b, /window.v1211Refresh/);
  assert.match(b, /setInterval\(function\(\)\{ if \(!document.hidden\) pullLive\(\); \}, 800\)/);
});

test('v12.12.0 instant sync + render bridge + mehraein header', () => {
  const app = read('../public/crm-app.js');
  const srv = read('../server.js');
  const b = read('../public/crm-bundle.js');
  const html = read('../public/index.html');
  ...
  assert.match(app, /CRM_APP_VERSION = "12.12.0"/);
  assert.match(srv, /const APP_VERSION = "12.12.0"/);
  assert.match(b, /window.v1212LiveSync/);
  assert.match(b, /window.v1212RenderBridge/);
  assert.match(html, /برنامه ویزیت و گزارشات \(مهر آیین نیک دارو\)/);
  assert.match(html, /TANIN TEB TAHA/);
  assert.match(html, /id="headerCompanyNameDisplay">طنین طب طاها/);
  assert.match(php, /not-modified/);
  assert.match(hub, /path=sync&target=render/);
  assert.match(b, /setInterval\(function\(\)\{ if \(!document.hidden\) pullLive\(\); \}, 800\)/);
});
