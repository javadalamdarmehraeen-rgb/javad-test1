...
test('next-chat GitHub handoff records exact publish truth and engine map', () => {
  assert.match(githubHandoff,/نسخه آماده سورس:[\s\S]*12\.08.0/);
...
test('v38 cache rescue automatically forces a fresh build without deleting CRM data', () => {
  ...
  assert.match(server,/"X-CRM-Build": APP_VERSION/);assert.match(server,/const APP_VERSION = "12\.08.0"/);
  ...
  assert.match(app,/CRM_BUILD_ACTIVE/);assert.match(app,/register\('\/sw\.js\?v=12\.08.0'/);
});
...
test('PWA activation is automatic and diagnostics never request manual refresh', () => {
  assert.match(app,/register\('\/sw\.js\?v=12\.08.0', \{ scope: '\/', updateViaCache: 'none' \}\)/);
...
test('v11.61.0 safe cache-hardening', () => {
  ...
  assert.match(app,/CRM_APP_VERSION = "12.08.0"/);
...
test('v11.72.0 durable target ops + mm gap + row number', () => {
  ...
  const css = read('../public/style.css');
  assert.match(app, /CRM_APP_VERSION = "12.08.0"/);
  assert.match(srv, /const APP_VERSION = "12.08.0"/);
...
test('v11.72.0 server-first paint + no-spin qty + per-row ops + dist achieved/remain', () => {
  ...
  assert.match(app, /CRM_APP_VERSION = "12.08.0"/);
  assert.match(srv, /const APP_VERSION = "12.08.0"/);
  assert.match(app, /register\('\/sw\.js\?v=12\.08.0'/);
...
test('v11.73.0 live state bind + delayed unveil + durable dist save + kill old ops hosts', () => {
  ...
  assert.match(app, /CRM_APP_VERSION = "12.08.0"/);
  assert.match(srv, /const APP_VERSION = "12.08.0"/);
  assert.match(sw, /const BUILD = "12.08.0"/);
  ...
  assert.match(html, /var BUILD="12.08.0"/);
  ...
  assert.match(login, /var BUILD="12.08.0"/);
...
test('v11.93.0 static netafraz + env hubs + sync-all + timeout hub', () => {
  ...
  assert.match(html, /crm-hub\.js\?v=12\.08.0/);
  assert.match(html, /crm-runtime\.js\?v=12\.08.0/);
...
test('v11.94.0 netafraz php api independent + leaflet root + no api 404 fake', () => {
  ...
  assert.match(app, /CRM_APP_VERSION = "12.08.0"/);
  assert.match(srv, /const APP_VERSION = "12.08.0"/);
  assert.match(html, /leaflet.css\?v=12\.08.0/);
  assert.match(html, /leaflet.js\?v=12\.08.0/);
...
test('v11.95.0 same version badge + origin-only hubs + taha company name', () => {
  ...
  assert.match(html, /نسخه ۱۲\.۰۸\.۰/);
  assert.match(html, /id="headerCompanyNameDisplay">طنین طب طاها/);
  assert.match(login, /نسخه ۱۲\.۰۸\.۰/);
...
test('v11.96.0 netafraz api/sync to render + gen stamp + no empty wipe', () => {
  ...
  assert.match(html, /crm-app\.js\?v=12\.08.0/);
});
...
test('v12.08.0 persist no wipe + one specialty field + no geo loop', () => {
  ...
  assert.match(b, /window.v1207FormFix/);
  assert.match(b, /doctorSpecialtyList/);
