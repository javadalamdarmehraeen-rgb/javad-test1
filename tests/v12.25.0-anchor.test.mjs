/**
 * v12.25.0 — نوبت ۱۴۶ (بر پایهٔ ۱۴۵)
...
const layerSrc = readFileSync(new URL('public/crm-v12.25.0.js', root), 'utf8');
...
/* ───────── ۱) بارگذاری و نسخه ───────── */
test('v12.25.0: لایهٔ پایانی پس از باندل بارگذاری می‌شود و نسخه در همهٔ سطوح یکی است', () => {
  const iBundle = indexSrc.indexOf('crm-bundle.js?v=12.25.0');
  const iLayer = indexSrc.indexOf('crm-v12.25.0.js?v=12.25.0');
  assert.ok(iBundle > 0 && iLayer > 0, 'هر دو اسکریپت در index.html');
  assert.ok(iLayer > iBundle, 'لایهٔ ۱۲.۲۰ آخرین اسکریپت است');
  assert.equal(pkg.version, '12.25.0');
  assert.match(bundle, /نسخه‌ی جاری: 12\.25\.0/);
  assert.match(serverSrc, /const APP_VERSION = "12\.25\.0"/);
  assert.match(phpSrc, /define\("CRM_APP_VERSION", "12\.25\.0"\)/);
});

/* ───────── ۲) قفلِ لنگر ───────── */
test('v12.25.0: ترتیبِ لنگر ضبط می‌شود و کادرِ لوکیشن جزوِ فیلدهایِ جابه‌جاشدنی نیست', () => {
...
test('v12.25.0: فیلدی که به پایینِ کادرِ لوکیشن پرتاب شود، به جایِ اصلی برمی‌گردد', () => {
  const s = pharmacyScene();
...
test('v12.25.0: ظرفِ فیلدهایِ سفارشی به انتهای گرید برود، ترتیبِ فرم نمی‌شکند', () => {
  const s = pharmacyScene();
  s.api.captureCanon('tab-pharmacies');
...
test('v12.25.0: تنها «شماره ترتیب در فرم» جایِ فیلد را عوض می‌کند، کادرِ لوکیشن سرِ جایش', () => {
  const s = pharmacyScene();
  s.api.captureCanon('tab-pharmacies');
...
test('v12.25.0: تنظیماتِ غیرِ ترتیب (عرض/فاصله/سطر) هیچ جابه‌جاییِ فیلد نمی‌سازند', () => {
  const s = pharmacyScene();
  s.api.captureCanon('tab-pharmacies');
...
/* ───────── ۳) ایتم‌هایِ طراح ───────── */
test('v12.25.0: «فاصله نسبت به فیلد قبلی/بعدی (میلی‌متر)» با تبدیلِ واقعی به پیکسل می‌نشیند', () => {
  const s = pharmacyScene();
  s.api.captureCanon('tab-pharmacies');
...
test('v12.25.0: عرض، ارتفاع، شمارهٔ سطر و «زیرِ هم» روی همان فیلد اعمال می‌شوند', () => {
  const s = pharmacyScene();
  s.api.captureCanon('tab-pharmacies');
...
test('v12.25.0: «نمایش در فرم»، «وابسته به فیلد» و «افزودن لحظه‌ای گزینه»', () => {
  const s = pharmacyScene();
  s.api.captureCanon('tab-pharmacies');
...
test('v12.25.0: «داخل کدام کادر؟» ذخیره می‌شود (پیش از این مقدارِ کشو دور ریخته می‌شد)', () => {
...
test('v12.25.0: هدرِ تک‌ردیفه، ساعت نخستین عضو، یک برچسبِ نسخه، دکمه‌هایِ پشتیبان', () => {
  const env = makeEnv({});
  const mk = env.node;
  ...
  const badge = mk('div', { id: 'crmBuildBadge', textContent: 'نسخه 12.25.0' });
  logoText.appendChild(badge);
  env.doc.body.appendChild(logoText);
  env.byId.set('crmBuildBadge', badge);
  const dup = mk('span', { id: 'v20VersionBadge', textContent: 'نسخه 12.25.0' });
...
test('v12.25.0: ساعت عرضِ ثابت دارد و ثانیه‌شمار فقط هنگامِ تغییرِ متن می‌نویسد', () => {
...
/* ───────── ۵) گاوصندوقِ تنظیمات ───────── */
test('v12.25.0: فیلدهایِ گم‌شده از پشتیبان برمی‌گردند، فیلدِ حذف‌شدهٔ عمدی نه', () => {
  const state = {
    customFields: {
...
test('v12.25.0: حذفِ عمدیِ فیلد، سنگ‌قبر می‌گیرد تا پشتیبان زنده‌اش نکند', () => {
  const state = {
...
test('v12.25.0: ریشه‌پاک‌کنِ نسخهٔ تازه، گاوصندوق و ترتیبِ مدیر را جارو نمی‌کند', () => {
...
test('v12.25.0: گاوصندوق روی سرورِ Node و هاستِ PHP هم ذخیره می‌شود', () => {
...
test('v12.25.0: خروجیِ اکسل همهٔ اطلاعاتِ تب را می‌دهد (ستونِ فیلدِ دارایِ مقدار هیچ‌وقت نمی‌افتد)', () => {
  const state = {
...
test('v12.25.0: همگام‌سازیِ خودکار هر ۱۰ ثانیه دست‌نخورده باقی است', () => {
...
test('v12.25.0: تاریخِ شمسی بدونِ ویرگول و بدونِ پرش ساخته می‌شود', () => {
  const env = makeEnv();
  env.win.getUnifiedFieldList = () => [];
  const api = env.api;
...
test('v12.25.0: ساعتِ هدر فقط در صورتِ تغییر بازنویسی می‌شود', () => {
  const src = layerSrc;
...
test('v12.25.0: قفلِ order+DOM — CSS `order` هم با ترتیبِ لنگر بازنویسی می‌شود', () => {
...
test('v12.25.0: کلیدِ چیدمانِ خودِ باندل با همان ترتیب پر می‌شود تا نجنگد', () => {
  const sc = pharmacyScene();
  sc.api.rebuildAllCanons();
  sc.api.enforceTab('tab-pharmacies');
...
test('v12.25.0: سرستون‌هایِ اکسل فارسی‌اند (dateAdded/fileName/managerPhone/repId)', () => {
  const env = makeEnv();
  env.win.getUnifiedFieldList = () => [];
  const api = env.api;
...
test('v12.25.0: طراح عددهایِ «واقعیِ» همان فیلد را نشان می‌دهد، نه ۲۲۰ برایِ همه', () => {
  const sc = pharmacyScene();
  const inputs = {};
...
test('v12.25.0: منویِ همبرگری با ✕ بسته می‌شود و کشو بالایِ هدر است', () => {
  const env = makeEnv();
  env.win.getUnifiedFieldList = () => [];
...
test('v12.25.0: «ارسال به رندر» از shim رد می‌شود و گزارشِ واقعیِ HTTP می‌دهد', () => {
...
test('v12.25.0: نسخه در همهٔ سطوح ۱۲.۲۱ است و لایه پس از باندل می‌آید', () => {
  assert.equal(pkg.version, '12.25.0');
  assert.match(indexSrc, /<script src="crm-v12\.25\.0\.js\?v=12\.25\.0"><\/script>/);
  const layerAt = indexSrc.indexOf('crm-v12.25.0.js');
  const bundleAt = indexSrc.indexOf('crm-bundle.js');
  assert.ok(bundleAt > 0 && layerAt > bundleAt, 'لایه پس از باندل');
  assert.match(bundle, /<li><b>12\.25\.0<\/b>/, 'بندِ تغییراتِ ۱۲.۲۱ در باندل');
...
test('v12.25.0: هم‌فرکانس‌سازیِ متا — موتورِ باندل همان orderِ ما را حساب می‌کند (پایانِ پرش)', () => {
  let saves = 0;
...
test('v12.25.0: کادرِ ساعت/تاریخ بلندتر است و نشان‌های هدر میخکوب', () => {
...
test('v12.25.0: همهٔ لوگوها و آیکون‌ها با نشانِ تازهٔ طنین طب طاها یکی شدند', () => {
...
test('v12.25.0: تابلو روانِ رویدادهای روز — آهسته، زیرِ کادر، با مناسبتِ امروز', () => {
  const env = makeEnv();
  env.win.getUnifiedFieldList = () => [];
  ...
  assert.match(cssSrc, /animation: crmTickerMove 60s linear infinite/, 'یک دور = ۶۰ ثانیه');
  assert.ok(!/id="crmTickerText2"/.test(layerSrc), 'کپیِ دومِ متن دیگر ساخته نمی‌شود');
  assert.match(layerSrc, /out = out\.filter\(function \(x, i\) \{ return out\.indexOf\(x\) === i; \}\);/, 'رویدادِ تکراری از متن حذف می‌شود');
  assert.match(cssSrc, /@keyframes crmTickerMove/, 'کی‌فریمِ روان');
});

test('v12.25.0: خروجیِ اکسل یک xlsx واقعی (ZIP معتبر) است — پسوندِ معتبر برایِ گوشی', () => {
  const env = makeEnv();
  env.win.getUnifiedFieldList = () => [];
...
test('v12.25.0: نگهبانِ ویزیت — اگر pull ویزیتِ فعال را پاک کرد، برمی‌گردد', () => {
  const env = makeEnv();
  env.win.getUnifiedFieldList = () => [];
...
test('v12.25.0: نقشهٔ تردد مراکزِ بینِ مسیر را با برچسبِ ویزیت‌شده/نشده می‌کارد', () => {
  const env = makeEnv();
  env.win.getUnifiedFieldList = () => [];
  const layers = [];
...
test('v12.25.0: هدرِ عمودیِ گوشی مرتب است و auto-update.php در بسته است', () => {
...
test('v12.25.0: تابلوی روان «داخلِ» هدرِ چسبان می‌نشیند و با اسکرول فریز است', () => {
  const env = makeEnv();
  const mk = env.node;
  const header = mk('header', { className: 'app-header' });
  env.doc.body.appendChild(header);
...
test('v12.25.0: پشتیبانِ خودکارِ «دیدنی» — هوکِ saveState + برچسبِ ساعتِ آخرین پشتیبان در هدر', () => {
  const env = makeEnv();
  let saved = 0;
  ...
  assert.match(layerSrc, /پشتیبانِ خودکارِ «دیدنی»: هر ۵ دقیقه[\s\S]{0,600}?\}, 300000\);/, 'بازهٔ ۵ دقیقه');
  assert.match(cssSrc, /\.v1224-backup-chip \{/, 'قالبِ برچسبِ پشتیبان');
});

test('v12.25.0: مالکیتِ کاملِ شروع/پایانِ ویزیت — GPS صریح، وضعیتِ دیدنی، ذخیرهٔ مسیر', () => {
  const env = makeEnv();
  const mk = env.node;
...
test('v12.25.0: ردِ اجازهٔ GPS پیامِ «دیدنی» می‌دهد و جریان نمی‌شکند', () => {
  const env = makeEnv();
  const mk = env.node;
  const box = mk('div', { id: 'visitStatusBox' });
  env.doc.body.appendChild(box);
...
test('v12.25.0: کارتِ انتقالِ داده بینِ رندر و نت‌افراز در تبِ عیب‌یابی', () => {
  const env = makeEnv();
  const mk = env.node;
...
test('v12.25.0: هوک‌ها و ابزارها در راه‌اندازی سیم‌کشی شده‌اند', () => {
...
test('v12.25.0: فیلدِ اجباری «یک» ستاره دارد، نه «**نام داروخانه*** *»', () => {
  const env = pharmacyScene();
  const mk = env.node;
  /* برچسبی که در رندرهایِ پیاپی ستاره جمع کرده + سه spanِ ستاره */
  const lab = env.gA.querySelector ? env.gA.querySelector('.form-label') : null;
  const label = lab || env.gA.children[0];
  label.childNodes = [{ nodeType: 3, nodeValue: '**نام داروخانه*** *' }];
  const s1 = mk('span', { className: 'req-star' });
  const s2 = mk('span', { className: 'req-star' });
  const s3 = mk('span', { className: 'req-star' });
  label.appendChild(s1); label.appendChild(s2); label.appendChild(s3);
  const before = env.api.fixStars1225();
  assert.ok(before >= 1, 'دستِ‌کم یک برچسب تمیز شد');
  assert.equal(label.childNodes[0].nodeValue.indexOf('*'), -1, 'هیچ «*» در متنِ برچسب نماند: ' + label.childNodes[0].nodeValue);
  assert.equal(label.querySelectorAll('.req-star').length, 1, 'فقط یک spanِ ستاره ماند');
  /* اجرایِ دوباره چیزی را خراب نمی‌کند */
  env.api.fixStars1225();
  assert.equal(label.querySelectorAll('.req-star').length, 1, 'پایدار است');
  assert.match(layerSrc, /try \{ fixStars1225\(\); \} catch \(e3\) \{\}/, 'در هر گذرِ نگهبان اجرا می‌شود');
});

test('v12.25.0: تابلوی روان ۶۰ ثانیه است و هر رویداد «یک‌بار» می‌آید', () => {
  const env = makeEnv();
  const mk = env.node;
  const header = mk('header', { className: 'app-header' });
  env.doc.body.appendChild(header);
  ...
  const bar = env.doc.getElementById('crmEventTicker');
  assert.ok(bar, 'تابلو ساخته شد');
  assert.ok(env.doc.getElementById('crmTickerText'), 'یک متن دارد');
  assert.equal(env.doc.getElementById('crmTickerText2'), null, 'کپیِ دوم ساخته نشد');
  /* اگر کپیِ دوم از نسخهٔ قبل در DOM مانده باشد، پاک می‌شود */
  const legacy = mk('span', { id: 'crmTickerText2' });
  bar.appendChild(legacy);
  env.api.buildTicker();
  assert.equal(env.doc.getElementById('crmTickerText2'), null, 'کپیِ مانده پاک شد');
  const txt = env.api.tickerText();
  const parts = txt.split('✦').map((x) => x.trim()).filter(Boolean);
  assert.equal(new Set(parts).size, parts.length, 'رویدادِ تکراری در متن نیست: ' + txt.slice(0, 90));
  assert.match(cssSrc, /animation: crmTickerMove 60s linear infinite/, '۶۰ ثانیه');
});

test('v12.25.0: موجیبیک پاک می‌شود و رشتهٔ اقلام فشرده و بی‌تکرار است', () => {
  const env = makeEnv();
  const mk = env.node;
  assert.equal(env.api.cleanFaText1225('â€Žâ€Žسافت ژل امگا 3\u200f'), 'سافت ژل امگا 3', 'نویزِ کدگذاری رفت');
  assert.equal(env.api.cleanFaText1225('  سافت   ژل '), 'سافت ژل', 'فاصله‌هایِ اضافه جمع شد');
  const pane = mk('section', { id: 'tab-orders' });
  const td = mk('td');
  const row = mk('tr'); row.appendChild(td);
  const tb = mk('tbody'); tb.appendChild(row);
  const tbl = mk('table'); tbl.appendChild(tb);
  pane.appendChild(tbl);
  env.doc.body.appendChild(pane);
  td.textContent = 'سافت ژل امگا 3 = تعداد کالا: 100 / تعداد جایزه: 10، سافت ژل امگا 3 = تعداد کالا: 100 / تعداد جایزه: 10، کپسول ملاتونین = تعداد کالا: 700 / تعداد جایزه: 70';
  const n = env.api.fixGiftCells1225();
  assert.equal(n, 1, 'یک خانه بازنویسی شد');
  const out = td.textContent;
  assert.equal(out.split('سافت ژل امگا 3').length - 1, 1, 'نامِ تکراری یک‌بار آمد: ' + out);
  assert.ok(out.includes('سافت ژل امگا 3 (100 کالا / 10 جایزه)'), 'قالبِ فشرده: ' + out);
  assert.ok(out.includes('کپسول ملاتونین (700 کالا / 70 جایزه)'), 'کالایِ دوم ماند');
  assert.ok(out.indexOf('= تعداد کالا:') === -1, 'قالبِ پرگویِ قدیمی رفت');
});

test('v12.25.0: کالایِ تکراری یکی می‌شود و کالایِ حذف‌شده با merge برنمی‌گردد', () => {
  const env = makeEnv();
  env.win.state.products = [
    { name: 'سافت ژل امگا 3', code: 1001 },
    { name: ' سافت ژل امگا 3 ', code: 1001 },
    { name: 'کپسول ملاتونین', code: 1007 }
  ];
  const dropped = env.api.productGuard1225();
  assert.ok(dropped >= 1, 'تکراری حذف شد');
  assert.equal(env.win.state.products.length, 2, 'دو کالایِ یکتا ماند');
  /* کاربر «کپسول ملاتونین» را حذف می‌کند */
  env.win.state.products = env.win.state.products.filter((p) => p.name.indexOf('ملاتونین') < 0);
  env.api.productGuard1225();
  assert.ok(env.mem.get('CRM_V1225_PRODUCT_TOMB'), 'سنگ‌قبرِ حذف ثبت شد');
  /* mergeِ سرور آن را برمی‌گرداند → لایه دوباره حذفش می‌کند */
  env.win.state.products = [{ name: 'سافت ژل امگا 3', code: 1001 }, { name: 'کپسول ملاتونین', code: 1007 }];
  const again = env.api.productGuard1225();
  assert.ok(again >= 1, 'کالایِ سنگ‌قبرگرفته دوباره حذف شد');
  assert.equal(env.win.state.products.some((p) => p.name.indexOf('ملاتونین') >= 0), false, 'برنگشت');
});

test('v12.25.0: هر فیلدِ تازهٔ «داروخانه‌ها» در «سفارشات» هم آینه می‌شود', () => {
  const env = makeEnv();
  env.win.state.customFields = {
    pharmacy: [
      { id: 'cf-pharmacy-1', label: 'طبقهٔ داروخانه', inputKind: 'simple', order: 7, options: [], showInForm: true },
      { id: 'cf-pharmacy-2', label: 'نوعِ همکاری', inputKind: 'select', order: 8, options: ['خرید', 'امانی'], showInForm: true }
    ],
    order: []
  };
  const added = env.api.mirrorPharmacyToOrders1225();
  assert.equal(added, 2, 'دو فیلد آینه شد');
  const dst = env.win.state.customFields.order;
  assert.equal(dst.length, 2, 'در سفارشات نشست');
  assert.equal(dst[0].label, 'طبقهٔ داروخانه', 'همان عنوان');
  assert.deepEqual(dst[1].options, ['خرید', 'امانی'], 'همان گزینه‌ها');
  assert.equal(dst[1].order, 8, 'همان ترتیب');
  assert.ok(dst[0].mirrorOf === 'cf-pharmacy-1', 'نسبتِ آینه ثبت شد');
  /* اجرایِ دوباره فیلدِ تکراری نمی‌سازد */
  assert.equal(env.api.mirrorPharmacyToOrders1225(), 0, 'تکراری ساخته نشد');
  assert.equal(env.win.state.customFields.order.length, 2, 'دو تا ماند');
  /* حذفِ مبدأ → آینه هم می‌رود */
  env.win.state.customFields.pharmacy = env.win.state.customFields.pharmacy.filter((f) => f.id !== 'cf-pharmacy-1');
  env.api.mirrorPharmacyToOrders1225();
  assert.equal(env.win.state.customFields.order.length, 1, 'آینهٔ حذف‌شده رفت');
  assert.equal(env.win.state.customFields.order[0].label, 'نوعِ همکاری', 'آن یکی ماند');
});

test('v12.25.0: در تبِ سفارشات فقط «یک» کادرِ جایگذاری خودکارِ داروخانه دیده می‌شود', () => {
  const env = makeEnv();
  const mk = env.node;
  const tab = mk('section', { id: 'tab-orders' });
  const a = mk('div', { id: 'crm1215OrderPick' });
  const b = mk('div', { id: 'crm1216TopPick' });
  tab.appendChild(a); tab.appendChild(b);
  env.doc.body.appendChild(tab);
  assert.equal(env.api.singlePickBox1225(), 1, 'کادرِ دوم پنهان شد');
  assert.equal(b.style.getPropertyValue('display'), 'none', 'پنهان است');
  assert.notEqual(a.style.getPropertyValue('display'), 'none', 'کادرِ اصلی دیدنی ماند');
  /* پنهان‌کردن است نه حذف → سازندهٔ باندل دوباره نمی‌سازد و پرش تمام می‌شود */
  assert.ok(env.doc.getElementById('crm1216TopPick'), 'گره هنوز هست (سازنده دوباره نمی‌سازد)');
  assert.equal(env.api.singlePickBox1225(), 0, 'پایدار است');
  assert.match(layerSrc, /other\.style\.setProperty\("display", "none", "important"\)/, 'پنهان با !important');
});

test('v12.25.0: «شمارهٔ ترتیبِ فرم و لیست» با ضبطِ دوبارهٔ لنگر می‌ماند', () => {
  const s = pharmacyScene();
  const before = orderOf(s.grid);
  /* کاربر فیلدِ دوم را به جایِ نخست می‌برد (کاری که safeOrderFieldsِ باندل می‌کند) */
  s.grid.insertBefore(s.gB, s.gA);
  assert.deepEqual(orderOf(s.grid), [before[1], before[0], before[2], before[3]], 'DOM جابه‌جا شد');
  /* پیش‌تر نگهبان لنگرِ کهنه را اعمال و جابه‌جایی را برمی‌گرداند؛ حالا لنگر تازه ضبط می‌شود */
  s.win._activeColTab = 'tab-pharmacies';
  s.api.recaptureAfterOrder1225('tab-pharmacies');
  s.timers.forEach((fn) => { try { fn(); } catch (e) {} });
  s.api.enforceAll();
  assert.deepEqual(orderOf(s.grid), [before[1], before[0], before[2], before[3]], 'ترتیبِ تازه ماند');
  /* هندلرِ راه‌اندازی هم به همین تابع وصل است، نه rebuildAllCanons */
  assert.match(layerSrc, /recaptureAfterOrder1225\(window\._activeColTab \|\| "tab-pharmacies"\);/, 'هندلرِ تغییرِ شماره');
  assert.ok(!/if \(!t\.classList\.contains\("col-order-input"\) && t\.id !== "colFieldOrder"\) return;/.test(layerSrc), 'هندلرِ کهنهٔ rebuildAllCanons رفت');
  assert.match(layerSrc, /col-listorder-input/, 'شمارهٔ ترتیبِ «لیست» هم پوشش دارد');
});

test('v12.25.0: پشتیبانِ خودکار هر ۵ دقیقه است (نه ۳۰ ثانیه)', () => {
  assert.match(layerSrc, /پشتیبانِ خودکارِ «دیدنی»: هر ۵ دقیقه[\s\S]{0,600}?\}, 300000\);/, 'بازهٔ ۵ دقیقه');
  assert.ok(!/\}, 30000\);/.test(layerSrc), 'بازهٔ ۳۰ ثانیهٔ قدیمی نمانده');
});

test('v12.25.0: «🗺 نمایش تردد» نقشه را می‌سازد و مسیر را می‌کشد', () => {
  const env = makeEnv();
  const mk = env.node;
  const pane = mk('section', { id: 'tab-rep-routes' });
  env.doc.body.appendChild(pane);
  env.win.state.visitTracks = [{
    repName: 'رضا', date: '1405/6/18', distance: 1234,
    path: [[35.70, 51.40], [35.71, 51.41], [35.72, 51.42]]
  }];
  const drawn = [];
  const mapStub = {
    invalidateSize() {}, removeLayer(l) { drawn.push('rm'); }, fitBounds() {}
  };
  env.win._mapRepRoutes = mapStub;
  env.win.L = {
    polyline(pts, o) { drawn.push(['line', pts.length]); return { addTo: () => ({}), getBounds: () => ({}) }; },
    marker(pt) { drawn.push(['marker', pt[0]]); return { addTo: () => ({ bindPopup() {} }) }; }
  };
  const tr = mk('tr');
  const c = mk('td', { textContent: 'رضا — 1405/6/18' });
  tr.appendChild(c);
  const ok = env.api.showTraffic1225(tr);
  assert.equal(ok, true, 'رسم موفق بود');
  assert.ok(drawn.some((d) => d[0] === 'line' && d[1] === 3), 'خطِ مسیر با ۳ نقطه کشیده شد');
  assert.equal(drawn.filter((d) => d[0] === 'marker').length, 2, 'نشانِ مبدأ و مقصد');
  const st = env.doc.getElementById('v1225TrafficStatus');
  assert.ok(st, 'کادرِ وضعیتِ دیدنی ساخته شد');
  assert.match(st.textContent, /✅ ترددِ «رضا» با 3 نقطه و 1234 متر/, 'گزارشِ دیدنی: ' + st.textContent);
});

test('v12.25.0: «🗺 نمایش تردد» به‌جایِ سکوت، دلیلِ روشن می‌دهد', () => {
  const env = makeEnv();
  const mk = env.node;
  const pane = mk('section', { id: 'tab-rep-routes' });
  env.doc.body.appendChild(pane);
  env.win.state.visitTracks = [];
  env.win.state.repRoutes = [];
  const tr = mk('tr');
  tr.appendChild(mk('td', { textContent: 'رضا — 1405/6/18' }));
  assert.equal(env.api.showTraffic1225(tr), false, 'بدونِ مسیر رسم نمی‌کند');
  assert.match(env.doc.getElementById('v1225TrafficStatus').textContent, /⚠️/, 'پیامِ دیدنیِ نبودِ مسیر');
  /* مسیرِ تک‌نقطه‌ای هم بهانهٔ روشن می‌گیرد */
  env.win.state.visitTracks = [{ repName: 'رضا', path: [[35.7, 51.4]] }];
  assert.equal(env.api.showTraffic1225(tr), false, 'تک‌نقطه رسم نمی‌شود');
  assert.match(env.doc.getElementById('v1225TrafficStatus').textContent, /کمتر از دو نقطه/, 'دلیلِ روشن');
  /* نقشهٔ نبود → پیامِ راهنما، نه سکوت */
  env.win.state.visitTracks = [{ repName: 'رضا', path: [[35.7, 51.4], [35.8, 51.5]] }];
  env.win.L = undefined;
  assert.equal(env.api.showTraffic1225(tr), false, 'بدونِ Leaflet رسم نمی‌کند');
  assert.match(env.doc.getElementById('v1225TrafficStatus').textContent, /نقشه آماده نشد/, 'راهنمایِ نبودِ نقشه');
  /* کلیکِ دکمه در فازِ capture مالِ لایه است */
  assert.match(layerSrc, /t\.closest\("\.crm1215-traffic-btn,#btnV20ShowRoute,\.v20-traffic-btn"\)/, 'مالکیتِ کلیک');
  assert.match(layerSrc, /showTraffic1225\(btn\.closest\("tr"\) \|\| null\);/, 'اجرایِ رسم');
  assert.match(cssSrc, /\.v1225-traffic-status \{/, 'قالبِ کادرِ وضعیت');
});
