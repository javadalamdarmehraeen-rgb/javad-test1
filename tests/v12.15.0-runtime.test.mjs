...
/* ───────── ۱) ترتیبِ پایدار ───────── */
test('v12.17.1: field order is normalised to unique rows (no ties → survives refresh)', (t) => {
  const env = withEnv(t, {
    state: {
      formFieldMeta: {
        pharmacies: {
...
test('v12.17.1: reverse geocode returns a real address and never raw coordinates', async (t) => {
...
test('v12.17.1: photon answers are turned into the same Persian address line', (t) => {
  const env = withEnv(t, {});
...
test('v12.17.1: the offline badge is corrected while the connection is healthy', async (t) => {
  const env = withEnv(t, {});
...
test('v12.17.1: saves go to an outbox and are flushed when the connection returns', async (t) => {
...
test('v12.17.1: order pick box shows same-named pharmacies with their address and fills the form', (t) => {
  const env = withEnv(t, {
    state: {
      pharmacies: [
...
test('v12.17.1: the pick box is inserted at the very top of the orders tab', (t) => {
...
/* ───────── ۶) نمایش تردد ───────── */
test('v12.17.1: every traffic row gets a "نمایش تردد" button that draws origin → destination', async (t) => {
  const env = withEnv(t, {
    state: {
...
/* ───────── ۷) طبقه و پلاک ───────── */
test('v12.17.1: homes list gains floor and plate columns', (t) => {
...
/* ───────── ۸) پیام‌رسان ───────── */
test('v12.17.1: messenger tokens, group names and destination numbers (multiple each)', (t) => {
...
/* ───────── ۹) قیمت‌گذاری ───────── */
test('v12.17.1: consumer price with VAT is restored after save (never reverts)', (t) => {
  const env = withEnv(t, {});
...
/* ───────── ۱۰) نوع فیلد ساعت ───────── */
test('v12.17.1: "ساعت" is an available field type and shows hour:minute only', (t) => {
  const env = withEnv(t, {});
...
/* ───────── ۱۳) دسترسیِ ریز ───────── */
test('v12.17.1: granular permissions are stored per tab and per action', (t) => {
...
test('v12.17.1: visit alarms are raised one day before for user, supervisor and manager', (t) => {
  const now = Date.now();
...
/* ───────── ۱۶) خروجی اکسل ───────── */
test('v12.17.1: exports get Persian headers and a row number', (t) => {
  const env = withEnv(t, {});
  let captured = null;
...
test('v12.17.1: excel column order follows the sequence defined in the columns tab', (t) => {
  const env = withEnv(t, {
    state: {
...
/* ───────── استاتیک ───────── */
test('v12.17.1: styling exists and the layer never blocks other tabs (idle scheduling)', (t) => {
