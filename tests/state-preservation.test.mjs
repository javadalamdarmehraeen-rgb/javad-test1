...
test('Shafaarad mappings and derived inventory column 10 are permanent', () => {
  ...
  assert.match(v20Source,/1001:1391911001/);assert.match(v20Source,/1005:1391911006/);assert.match(v20Source,/1006:1391911005/);assert.match(v20Source,/1007:1391902006/);assert.match(v20Source,/shafaDbCode/);
  const c2={result:null,SHAFA_CODE_MAP:{1001:1391911001,1005:1391911006,1006:1391911005,1007:1391902006},st:()=>({products:[{code:1001,name:'سافت ژل امگا 3',shafaDbCode:1391911001},{code:1005,name:'سافت ژل امگا وومن',shafaDbCode:1391911006},{code:1006,name:'سافت ژل امگا من',shafaDbCode:1391911005},{code:1007,name:'کپسول ملاتونین',shafaDbCode:1391902006}]}),norm:v=>String(v||'').replace(/\s+/g,'')};vm.createContext(c2);c2.enDigits=v=>String(v);vm.runInContext(`${extract('normalizeDbCode')};${extract('canonicalProduct')};result=canonicalProduct`,c2);assert.equal(c2.result('نام خام','1391911006','shafaarad'),'سافت ژل امگا وومن');assert.equal(c2.result('نام خام','1391911005','shafaarad'),'سافت ژل امگا من');assert.equal(c2.result('نام خام','1.391911001E9','shafaarad'),'سافت ژل امگا 3');assert.equal(c2.result('نام خام','1391902006','shafaarad'),'کپسول ملاتونین');assert.equal(c2.result('نامشخص','','daya'),null);
});
