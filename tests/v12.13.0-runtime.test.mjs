/**
 * اجرای واقعی لایه v12.15.0 — فرمان درخواست‌ها، HTTPS، GPS، پاک‌سازی یک‌باره.
...
assert.ok(layer.length > 1500, 'لایه v12.15.0 پیدا نشد');
...
test('v12.15.0 runtime: request governor spaces background calls (healthy host)', async (t) => {
...
test('v12.15.0 runtime: a 5xx host is quarantined instantly (no storm, no wait)', async (t) => {
...
test('v12.15.0 runtime: user saves are never queued or delayed', async (t) => {
  const env = makeEnv({ status: 200 });
  t.after(() => env.cleanup());
  const t0 = Date.now();
...
test('v12.15.0 runtime: one-time cleanup purges caches but never user data', () => {
  const env = makeEnv();
  env.cleanup();
