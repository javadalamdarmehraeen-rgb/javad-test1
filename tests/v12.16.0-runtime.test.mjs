...
test('v12.17.1: applyOrder پاک‌سازیِ order قدیمی و بازچینیِ واقعیِ فرم و لیست', () => {
  const env = boot();
  const calls = [];
...
test('v12.17.1: هنگامِ تایپ، بازچینی اجرا نمی‌شود (فیلد نمی‌پرد)', () => {
  const env = boot();
  const pane = env.doc.createElement('section');
  pane.id = 'tab-pharmacies';
  env.doc.body.appendChild(pane);
...
test('v12.17.1: هیچ مختصاتِ پیش‌فرضی (تهران) در شکستِ GPS جایگزین نمی‌شود', () => {
...
test('v12.17.1: شکستِ GPS فقط پیام می‌دهد و هیچ مقداری در فرم نمی‌نویسد', async () => {
  const env = boot();
...
test('v12.17.1: آدرسِ همان نقطه نوشته می‌شود (وارسو، نه تهران)', async () => {
  const env = boot();
...
test('v12.17.1: فقط داروخانه‌های هم‌نام با نامِ نوشته‌شده نمایش داده می‌شوند', () => {
  const env = boot({
    pharmacies: [
...
test('v12.17.1: کادرِ هم‌نام جایگزینِ کادرِ قدیمی و در بالای تب سفارشات است', () => {
...
test('v12.17.1: نقاطِ مسیر از آرایه، از اشیاء و از لاگِ فعالیت خوانده می‌شوند', () => {
  const env = boot({
...
test('v12.17.1: بدون نقطه، پیامِ درست می‌دهد (نه «نقشه در دسترس نیست»)', () => {
  const env = boot({});
...
test('v12.17.1: طبقه و پلاک در رکوردِ منزل ذخیره می‌شود', () => {
...
test('v12.17.1: ویرایشِ قیمت مصرف‌کننده با ارزش افزوده، بقیه را درست می‌سازد', () => {
  const env = boot({
    products: [{
...
test('v12.17.1: فقط بعد از سه شکستِ پیاپی آفلاین می‌شویم و با یک پاسخِ موفق برمی‌گردیم', async () => {
  const env = boot({});
  let mode = 'fail';
...
/* ───────── ۸) سامانه پیامکی ─────────────────────────────────────────────── */
test('v12.17.1: شماره‌ها استاندارد و تولد از پرونده خوانده می‌شود', () => {
  const env = boot({});
...
test('v12.17.1: پیامِ تبریک با نامِ همان داروخانه/پزشک در صف می‌نشیند', () => {
  const now = new Date();
...
test('v12.17.1: تب سامانه پیامکی در فهرستِ تب‌ها و بدنهٔ صفحه وجود دارد', () => {
...
test('v12.17.1: سه بخشِ مستقلِ سامانه پیامکی در تب ترسیم می‌شود', () => {
...
/* ───────── ۹) نسخه یک‌دست است ───────────────────────────────────────────── */
test('v12.17.1: شمارهٔ نسخه در پرونده‌های اصلی یک‌دست است', () => {
  ...
  files.forEach((f) => {
    ...
    assert.ok(/12\.17.1/.test(txt), f + ' باید 12.17.1 را داشته باشد');
  });
  assert.ok(htmlSrc.includes('نسخه 12.17.1') || /12\.17.1/.test(htmlSrc), 'نسخه در صفحه است');
});
