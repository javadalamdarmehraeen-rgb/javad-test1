...
test('v12.17.1: نسخه در همه‌ی سطوح 12.17.1 است و README بنرِ نسخه دارد', () => {
  assert.match(readmeSrc, /نسخه‌ی جاریِ این ریپو \(GitHub main\): \*\*12\.21.0\*\*/, 'بنرِ صدرِ README');
  assert.match(htmlSrc, /BUILD="12\.21.0"/, 'BUILD در index.html');
  assert.match(serverSrc, /const APP_VERSION = "12\.21.0"/, 'APP_VERSION در server.js');
  const pkg = JSON.parse(readFileSync(new URL('package.json', root), 'utf8'));
  assert.equal(pkg.version, '12.21.0');
  assert.match(apiPhp, /define\("CRM_APP_VERSION", "12\.21.0"\)/, 'نسخه در api.php');
