...
# 94. NO-VPN RULE (PERMANENT — USER-MANDATED 2026-08-31, نوبت ۱۲۸)

برنامه باید با VPN روشن **و** خاموش بالا بیاید و کارش را انجام بدهد:

1. هیچ درخواست بین‌دامنه‌ای (رندر/نت‌افرازِ دیگر/کاشی/ژئوکد) مجاز نیست بیش از
   ۶ ثانیه معطل بماند: `AbortController` + تایم‌اوت کوتاه + `catch` کامل.
2. هیچ درخواست بین‌دامنه‌ای در مسیر بالا آمدن برنامه `await` نمی‌شود؛ همه پس‌زمینه‌اند.
3. `navigator.onLine === false` یعنی حتی یک درخواست خارجی هم فرستاده نشود.
4. نبود کاشی نقشه یا ژئوکد فقط یک پیام دوستانه است؛ ثبت، ذخیره و گزارش باید کار کند.
5. منبع حقیقتِ داده همان origin است (api.php روی نت‌افراز / سرور روی رندر).

# 95. THREE-DOMAIN SYNC RULE (PERMANENT — USER-MANDATED 2026-08-31, نوبت ۱۲۸)

دامنه‌های فعال: `https://javad-test1.onrender.com`، `https://mehraeinpharma.ir`،
`https://ndcohub.com`. در گوشی و ویندوز همگام باید بین هر سه برقرار باشد:

1. فهرست در `window.CRM_HUB_DOMAINS` و `window.v1212Peers()` است و خودِ دامنه را
   شامل نمی‌شود؛ `ndcohub.ir` (گواهی نامعتبر) از چرخه همگام خارج است.
2. همگام = کشیدن از هر هم‌دامنه → ادغام شناسه‌محور (جدیدتر برنده) → فرستادن ادغام
   به همان دامنه‌ها. ادغام هرگز تعداد رکوردهای ثبت‌شده را کم نمی‌کند.
3. نت‌افراز علاوه بر مرورگر، مسیر سروری `/api/sync?target=all` را هم دارد.
4. هر دامنه جدید باید هم‌زمان در این فایل‌ها ثبت شود: `crm-hub.js`، `crm-bundle.js`،
   `crm-app.js`، `api.php`، `server.js`، `build-static.js`، `api-config.json`،
   `api-config.php`، `.htaccess`، `render.yaml` و CIها.

# 96. HEADER BLOCK RULE (PERMANENT — USER-MANDATED 2026-08-31, نوبت ۱۲۸)

متن کنار لوگو دقیقاً سه خط و با همین ادبیات باشد:

1. `برنامه ویزیت و گزارشات (مهر آیین نیک دارو)` ← `#headerCompanyNameDisplay`
2. `نسخه <VER>` با **ارقام لاتین** ← `#crmBuildBadge`
3. `طنین طب طاها  TANIN TEB TAHA` (دو فاصله) ← `#headerBrandLine`

شماره نسخه همیشه از `window.CRM_APP_VERSION` خوانده می‌شود و با `MutationObserver`
روی بلوک `.logo-text` در برابر رنگ‌آمیزی‌های قدیمی قفل می‌شود. تغییر این متن
بدون دستور صریح کاربر ممنوع است.

## نوبت ۱۲۸ — نسخه ۱۲.۱۲.۰
سه نقص رفع شد؛ سربرگ سه‌خطی؛ بدون VPN؛ سه دامنه با همگام کامل.
