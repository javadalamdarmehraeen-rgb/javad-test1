...
**نسخه آماده سورس:** `12.25.0` (فایل واحد crm-bundle با لایه‌های تا v12.18 + لایهٔ پایانیِ `public/crm-v12.25.0.js` + موتورِ ورودِ crm-entry-engine.js + مویرگِ همگامِ چنددستگاهی — فایل‌های crm-features-v9 تا v20 حذف شده‌اند)
**نسخه chat.arena پس از بازسازی این نوبت:** `2.21`
**شاخه اجباری این جلسه:** `arena/01a080f0-javad-test1`

## 0.56) وضعیت نوبت ۱۵۲ — نسخه 12.25.0: یک ستاره + تابلوی ۶۰ ثانیهٔ تک‌رویداد + کالایِ بی‌بازگشت + آینهٔ فیلدها + ترتیبِ ماندنی + نمایشِ تردد ✅

| # | شکایت | ریشه/پاسخ |
|---|---|---|
| ۱ | فیلدهایِ ستاره‌دار دوستاره‌اند («**نام داروخانه*** *») | متنِ برچسب ستاره جمع می‌کرد و چند `.req-star` ساخته می‌شد → `fixStars1225()` در هر گذرِ نگهبان: حذفِ همهٔ `*`هایِ متن + نگه‌داشتنِ یک span |
| ۲ | تابلو ۱۲۰ ثانیه است و هر رویداد دوبار می‌آید | کپیِ دومِ متن (`#crmTickerText2`) برایِ حلقهٔ بی‌درز ساخته می‌شد → حذفِ کپیِ دوم + حذفِ رویدادِ تکراری در `tickerText()` + `60s` در CSS |
| ۳ | نام‌ها موجیبیک‌اند و رشتهٔ «سافت ژل امگا 3 = تعداد کالا: 100 / تعداد جایزه: 10، …» پرگو و تکراری است | `cleanFaText1225()` (نویزِ کدگذاری، بدونِ دست‌زدن به `«»`/`®`/خط‌تیره) + `fixGiftCells1225()` (قالبِ فشرده، هر نام یک‌بار) |
| ۴ | نامِ کالاها دوبار است و هر چه حذف می‌کنم برمی‌گردد (مخصوصاً رندر) | mergeِ سرور حذف را واگردانی می‌کرد → `productGuard1225()`: حذفِ تکراری بر پایهٔ نامِ نرمال + سنگ‌قبرِ `CRM_V1225_PRODUCT_TOMB` |
| ۵ | هر فیلدِ تازهٔ داروخانه‌ها باید در سفارشات هم باشد | `mirrorPharmacyToOrders1225()`: آینه با `mirrorOf` + حذفِ آینه با حذفِ مبدأ + `applyFullFormLayout("tab-orders")` |
| ۶ | دو کادرِ «جایگذاری خودکارِ داروخانه» و پرشِ یکی | دو سازنده (`#crm1215OrderPick` و `#crm1216TopPick`) هر دو در `tab.firstChild` → کادرِ دوم **پنهان** (نه حذف) تا سازنده‌اش ساکت بماند |
| ۷ | شمارهٔ ترتیبِ فرم و لیست کار نمی‌کند | نگهبان هر 900ms لنگرِ آغازین را بازمی‌گرداند → `recaptureAfterOrder1225()` پس از تغییر (1100ms/2500ms) و حذفِ `rebuildAllCanons` |
| ۸ | پشتیبانِ خودکار هر ۵ دقیقه باشد | بازهٔ دوره‌ای 30000 → **300000** میلی‌ثانیه |
| ۹ | «🗺 نمایش تردد» هیچ عکس‌العملی ندارد | نقشه ساخته نشده بود/مسیر پیدا نمی‌شد/پیام در alert خاموش → مالکیتِ capture، `openRoutesTab1225`، `ensureRoutesMap1225`، رسمِ مبدأ→مقصد، گزارش در `#v1225TrafficStatus` |

**تست:** `npm test` → **290/290**. **ZIP:** `namayandeelmi-v12.25.0.zip`.

## 0.55) وضعیت نوبت ۱۵۰ — نسخه 12.24.0: تابلوی فریز + پشتیبانِ دیدنی + مالکیتِ ویزیت + انتقالِ رندر↔نت‌افراز ✅

| # | شکایت | ریشه/پاسخ |
|---|---|---|
| ۱ | تاریخِ هدر هنوز «1,405» با ویرگول است و می‌پرد | ریشه: هاست‌ها کدِ قدیمیِ `main` را اجرا می‌کردند چون PR ادغام نشده بود (کدِ لایه بی‌ویرگول است). پاسخ: ادغامِ PR #9 به `main` + خودبروزرسانیِ نت‌افراز |
| ۲ | هر نسخه، فیلدهایِ افزوده/جابه‌جاشده می‌پرد | پاسخ: `hookSaveStateForBackup()` (هر `saveState` → `saveVault()` با 2.5s تأخیر) + بازهٔ 30s + فایل‌های داده هرگز داخلِ ZIP نیستند |
| ۳ | انتقالِ اطلاعاتِ رندر به نت‌افراز ممکن نیست | پاسخ: کارتِ `#v1224TransferCard` در تبِ عیب‌یابی — کپیِ کامل از رندر (`GET /api/state`)، ارسالِ کامل به نت‌افراز/رندر (`POST /api/state?__v12183=1`) با گزارشِ واقعیِ HTTP |
| ۴ | تابلویِ روان موقعِ اسکرول تکان می‌خورد | پاسخ: `buildTicker()` نوار را به `.app-header` (sticky, z-index 1000) منتقل می‌کند + `.app-header .crm-event-ticker` تمام‌عرض → فریز با هدر |
| ۵ | شروع/پایانِ ویزیت و رصدِ آن درست نشد | پاسخ: لایه مالکِ کاملِ دو دکمه در فازِ capture (`stopImmediatePropagation`)؛ `watchPosition` + `getCurrentPosition` صریح + بیمهٔ 5s؛ وضعیتِ GPS در `#visitStatusBox` (🛰/⛔/⚠️)؛ پایان → `repRoutes`+`visitTracks`+آمار+نقشه |
| ۶ | پشتیبانِ اتوماتیک فعال نیست، باید دستی گرفت | پاسخ: بازهٔ 30s + هوکِ `saveState` + برچسبِ `#v1224BackupChip` («🛡 HH:MM:SS») کنارِ ساعتِ هدر + `CRM_V1224_LASTBACKUP` |

**تست:** `npm test` → **280/280**. **ZIP:** `namayandeelmi-v12.25.0.zip`.

## 0.54) وضعیت نوبت ۱۴۸ — نسخه 12.23.0: تابلو روان تقویم + هدر گوشی + نگهبان ویزیت + نقشهٔ تردد + xlsx واقعی + خودبروزرسانی ✅

| # | شکایت | ریشه/پاسخ |
|---|---|---|
| ۱ | تغییراتِ نت‌افراز روی گوشی/رندر نمی‌نشیند + لوگو در نت‌افراز قدیمی است | کدِ نت‌افراز قدیمی مانده (آپلودِ دستی نشده). `auto-update.php` + GitHub Release: خودبروزرسانیِ یک‌کلیکی؛ همگامِ داده از قبل سه‌هابی است |
| ۲ | کادرِ بالا در حالتِ عمودیِ گوشی بهم‌ریخته | `@media (max-width:640px) and (orientation:portrait)`: دو ردیفِ مرتب، ساعت تمام‌عرض |
| ۳ | رویدادهای روزِ تقویمِ شمسی به‌صورتِ تابلو روانِ آهسته زیرِ کادر | `.crm-event-ticker` + بیش از ۱۳۰ مناسبت + انیمیشنِ ۱۲۰ ثانیه‌ای |
| ۴ | کاما در «1,405» و پرشِ عدد (در نت‌افراز) | همان کدِ قدیمیِ نت‌افراز؛ با خودبروزرسانی به ۱۲.۲۳ (که clockDate دست‌ساز دارد) تمام می‌شود |
| ۵ | شروع/پایان ویزیت چیزی ثبت نمی‌کند (۰ متر/۰ دقیقه/۰ نقطه) | نگهبانِ سایهٔ محلی + آمارِ جلسهٔ تمام‌شده در کادر |
| ۶ | نمایش تردد باید خطِ مسیر + مراکزِ ویزیت‌شده/نشده را نشان دهد | `drawRouteCenters()` با برچسبِ دائمی روی Leaflet |
| ۷ | پسوندِ اکسل روی گوشی نامعتبر | `buildXlsx()` = ZIP واقعیِ xlsx با CRC32، sheet راست‌به‌چپ |

**تست:** `npm test` → **274/274** (۵ تستِ تازه: تابلو، xlsx، نگهبانِ ویزیت، کارت‌های نقشه، CSS/خودبروزرسانی).
...
**تست:** `npm test` → **۲۶۹/۲۶۹** (در زمانِ انتشارِ 12.23.0). **ZIP:** `namayandeelmi-v12.23.0.zip`.
...
**تست:** `npm test` → **۲۶۶/۲۶۶** (۹ تستِ تازه در `tests/v12.25.0-anchor.test.mjs`، از جمله «هیچ سرستونِ لاتینی در خروجی نیست» و «orderِ هر گره = ترتیبِ DOM»).
**فایل‌ها:** `public/crm-v12.25.0.js` (جانشینِ `crm-v12.20.0.js`، با `window.v1221Api` و کلیدهایِ حافظهٔ بی‌تغییر)، `public/style.css`، `public/index.html`، `package.json`، `public/sw.js`، `public/crm-app.js`، `public/crm-bundle.js` (فقط رشته‌هایِ نسخه + بندِ تغییرات).
...
رفع (لایهٔ پایانیِ `public/crm-v12.25.0.js` + چهار ویرایشِ کوچکِ هدفمند):
