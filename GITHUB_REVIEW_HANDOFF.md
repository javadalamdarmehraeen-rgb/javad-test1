...
**نسخه آماده سورس:** `12.18.4` (فایل واحد crm-bundle با لایه‌های تا v12.18 + موتورِ ورودِ crm-entry-engine.js + مویرگِ همگامِ چنددستگاهی — فایل‌های crm-features-v9 تا v20 حذف شده‌اند)
**نسخه chat.arena پس از بازسازی این نوبت:** `2.21`
**شاخه اجباری این جلسه:** `arena/01a07288-javad-test1`

## 0.47) وضعیت نوبت ۱۴۱ — نسخه 12.18.4: دیوارِ آتشِ چیدمانی (رفعِ پرشِ فیلدها با هر نوسانِ آنلاین/آفلاین) ✅
...
**تست:** `tests/v12.18.4-stability.test.mjs` — ۳ تست (جایگاهِ فایروال درِ انتهایِ باندل + بعد از همهٔ نسلها؛ اجرایِ واقعیِ بلوکِ فایروال درِ `node:vm` باِ fetch جعلی و سنجشِ skip/جعلی/عبور؛ استقلالِ لولهٔ rawXhrِ مویرگ). رگرسیونِ کامل: **۲۳۴/۲۳۴**.
