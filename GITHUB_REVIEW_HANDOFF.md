...
**نسخه آماده سورس:** `12.14.0` (فایل واحد crm-bundle + آینه crm-features-v20)
...
```text
Source tested:             yes — ۱۲۸/۱۲۸ تست رگرسیون + ۴/۴ app-smoke + ۱۸/۱۸ runtime + ۴/۴ app-smoke + ۴/۴ runtime لایه v12.14.0
...
1. **رفع نقص ۱ — نسخه نامتوازن:** `package.json`، `crm-app.js`، `index.html`،
   ...
   همگی روی `12.14.0` یکسان شدند (پیش از این ۱۲.۰۹.۳/۱۲.۰۹.۰ قاطی بود).
...
4. **سربرگ سه‌خطی کنار لوگو:** `برنامه ویزیت و گزارشات (مهر آیین نیک دارو)` /
   `نسخه 12.14.0` (ارقام لاتین) / `طنین طب طاها  TANIN TEB TAHA` — با
...
$ node --test tests/v12.14.0-runtime.test.mjs   # ۳/۳ سبز
...
| اقدام | اثر |
|---|---|
| دارایی‌های نسخه‌دار (`?v=12.14.0`) با `Cache-Control: immutable` یک سال کش می‌شوند (سرور، `.htaccess`، خروجی استاتیک) | رفرش دیگر دارایی‌ها را نمی‌کشد |
...
node --test tests/v12.14.0-runtime.test.mjs  → ۱۱/۱۱ سبز
...
curl -I "/crm-app.js?v=12.14.0"               → Cache-Control: public, max-age=31536000, immutable
