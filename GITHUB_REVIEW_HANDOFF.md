...
**نسخه آماده سورس:** `12.24.0` (فایل واحد crm-bundle با لایه‌های تا v12.18 + لایهٔ پایانیِ `public/crm-v12.24.0.js` + موتورِ ورودِ crm-entry-engine.js + مویرگِ همگامِ چنددستگاهی — فایل‌های crm-features-v9 تا v20 حذف شده‌اند)
...
**تست:** `npm test` → **280/280**. **ZIP:** `namayandeelmi-v12.24.0.zip`.
...
**تست:** `npm test` → **۲۶۶/۲۶۶** (۹ تستِ تازه در `tests/v12.24.0-anchor.test.mjs`، از جمله «هیچ سرستونِ لاتینی در خروجی نیست» و «orderِ هر گره = ترتیبِ DOM»).
**فایل‌ها:** `public/crm-v12.24.0.js` (جانشینِ `crm-v12.20.0.js`، با `window.v1221Api` و کلیدهایِ حافظهٔ بی‌تغییر)، `public/style.css`، `public/index.html`، `package.json`، `public/sw.js`، `public/crm-app.js`، `public/crm-bundle.js` (فقط رشته‌هایِ نسخه + بندِ تغییرات).
...
رفع (لایهٔ پایانیِ `public/crm-v12.24.0.js` + چهار ویرایشِ کوچکِ هدفمند):
