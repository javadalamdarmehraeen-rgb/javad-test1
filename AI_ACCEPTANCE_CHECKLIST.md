...
## نوبت ۱۲۸ — نسخه ۱۲.۱۲.۰ (درخواست کاربر: رفع سه نقص + متن کنار لوگو + بدون VPN + سه دامنه)

بندهای پرامپت کاربر و راستی‌آزمایی هر بند:

- [x] بند ۱ — «اول سه نقص واقعی که پیدا شد حلش کن»:
  - نقص ۱ (نسخه نامتوازن): ۹ فایل نسخه یکسان `12.13.0` — تأییدشده با ۹ آزمون فایل‌به‌فایل
    و health زنده سرور (`"version":"12.13.0"`).
  - نقص ۲ (`public/index.php`): فایل ساخته شد + ثبت در `OFFICIAL_FILELIST.txt` +
    آزمون وجود/محتوا؛ اجرای واقعی روی هاست = «در انتظار تأیید کاربر» (PHP در این محیط نیست).
  - نقص ۳ (انتظارات تست‌ها): ۹۱ الگوی نسخه + ۵ آزمون قدیمی اصلاح شد؛
    نتیجه ۱۳۸/۱۳۹ (مورد باز مربوط به همین فایل آرشیو است که در این نوبت به‌روز می‌شود).
- [x] بند ۲ — متن کنار لوگو دقیقاً سه خط:
  خط ۱ `برنامه ویزیت و گزارشات (مهر آیین نیک دارو)`، خط ۲ `نسخه 12.13.0`
  (ارقام لاتین)، خط ۳ `طنین طب طاها  TANIN TEB TAHA` — تأییدشده در HTML سرو‌شده
  از سرور واقعی (`/panel` ۲۰۰) و با اجرای واقعی لایه در DOM شبیه‌سازی‌شده (۴/۴).
- [x] بند ۳ — بالا آمدن با VPN روشن یا خاموش:
  تایم‌اوت ۶ ثانیه برای همه درخواست‌های بین‌دامنه‌ای، `navigator.onLine === false`
  بدون ارسال، کاهش تایم‌اوت‌های ۲۰ ثانیه‌ای قدیمی به ۶ ثانیه، پیام دوستانه روی نقشه
  هنگام نبود کاشی، تایم‌اوت ۶ ثانیه برای ژئوکد در `server.js`.
  مدرک: آزمون اجرایی رد شدن واقعی درخواست پس از تایم‌اوت + عدم ارسال در حالت آفلاین.
- [x] بند ۴ — دامنه سوم نت‌افراز `ndcohub.com`:
  ثبت در `crm-hub.js`، `crm-bundle.js`، `crm-app.js`، `api.php` (CORS + `peer_hosts`
  + `/api/sync?target=all`)، `server.js` (هاست مجاز + هاب‌ها + CSP)، `build-static.js`
  (hubs + CORS چنددامنه‌ای)، `api-config.json/php`، `.htaccess`، `render.yaml`،
  CIها، و `OFFICIAL_FILELIST.txt`.
- [x] بند ۵ — همگام بین هر سه دامنه در گوشی و ویندوز:
  لایه مرورگر هر دامنه را می‌کشد/می‌فرستد (ادغام شناسه‌محور، بدون پاک‌شدن داده)،
  PHP نت‌افراز هم `/api/sync?target=all` دارد؛ فهرست هم‌دامنه‌ها روی هر سه origin
  آزمون شد (خود دامنه حذف می‌شود و `ndcohub.ir` به‌خاطر گواهی خراب نمی‌آید).
- [x] آزمون نهایی پیش از ZIP: ۱۳۸/۱۳۹ تست + ۴/۴ app-smoke (سرور واقعی) +
  ۴/۴ آزمون اجرایی لایه جدید + `npm run build-static` موفق + syntax همه JSها.
