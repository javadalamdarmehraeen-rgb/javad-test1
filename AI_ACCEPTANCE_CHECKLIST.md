...
- [x] بند ۱ — «اول سه نقص واقعی که پیدا شد حلش کن»:
  - نقص ۱ (نسخه نامتوازن): ۹ فایل نسخه یکسان `12.16.0` — تأییدشده با ۹ آزمون فایل‌به‌فایل
    و health زنده سرور (`"version":"12.16.0"`).
...
- [x] بند ۲ — متن کنار لوگو دقیقاً سه خط:
  خط ۱ `برنامه ویزیت و گزارشات (مهر آیین نیک دارو)`، خط ۲ `نسخه 12.16.0`
