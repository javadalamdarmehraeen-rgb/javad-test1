...
نسخه برنامه همچنان 11.66.0. اگر کاربر گفت «زیپ با فرمت دیگری ظاهر شده»، ZIP را PKZIP ویندوز بساز و صفحه دانلود پورت 8000 را نشان بده؛ present_file خام ZIP در Arena ممکن است سفید/باینری باشد. نسخه را بدون تغییر کد bump نکن.
...
## نوبت ۱۳۰ — نسخه ۱۲.۱۴.۰

رفع «This site can't be reached / ERR_CONNECTION_CLOSED» در رفرشِ ساده و سخت
(فرمان ترافیک + کشِ immutableِ دارایی‌های نسخه‌دار + لغوِ درخواست‌ها هنگام خروج)
و شتابِ دکمهٔ «موقعیت فعلی من» با همان دقتِ ریزآدرس (GPSِ تطبیقی + ژئوکدِ موازی).

## نوبت ۱۳۱ — نسخه ۱۲.۱۵.۰

شانزده اصلاح: ترتیبِ پایدارِ فیلدها، آدرس واقعی به‌جای مختصات، آنلاینِ پایدار،
جایگذاری خودکارِ بالای صفحه با آدرس، کلید نمایش تردد، طبقه/پلاک منزل، پیام‌رسانِ
چندگانه، تثبیتِ قیمت مصرف‌کننده، نوع فیلدِ ساعت، بوت آفلاین + ارسال خودکار،
زمان‌بندیِ idle، دسترسیِ ریز، زمان ویزیت + آلارم، و خروجی اکسلِ فارسی.
...
3. هر دامنه جدید باید هم‌زمان در ۱۱ فایل ثبت شود (فهرست در قانون ۹۵).
...
6. در انتظار تأیید کاربر: اجرای واقعی `public/index.php` روی هاست نت‌افراز،
   و production health پس از push (`/api/health` باید 12.15.0 باشد).
7. قانون ۹۷ (فرمان درخواست‌ها): هیچ درخواست پس‌زمینه‌ای را بدون گذر از `governed()`
   صدا نزن؛ ذخیرهٔ کاربر هرگز در صف نمی‌ماند.
8. قانون ۹۹ (فرمان ترافیک): سقف ۶ درخواستِ پس‌زمینه در دقیقه، ۲۵ ثانیه سکوتِ آغازین،
   و لغوِ درخواست‌های باز در `pagehide` را نقض نکن.
9. قانون ۱۰۰ (موقعیت‌یابی سریع): خروجِ تطبیقیِ GPS (≤۱۰ متر / ≤۳۰ متر پس از ۲.۵ ثانیه)
   و ژئوکدِ موازی با کش؛ `zoom=18` را هرگز پایین نیاور.
10. قانون ۱۰۱ (بدون مزاحمت): هر کارِ سنگین باید با `requestIdleCallback` و در تکه‌های
   ≤۸ میلی‌ثانیه باشد تا تب‌های دیگرِ مرورگر قفل نشوند.
11. قانون ۱۰۲ (دستورِ PowerShell): در دستورِ پایانِ هر چت، `npm install` را نیاور.
