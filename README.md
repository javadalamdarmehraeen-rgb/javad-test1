...
> # 🏷️ نسخه‌ی جاریِ این ریپو (GitHub main): **12.20.0**
>
> **تغییراتِ 12.20.0 (نوبت ۱۴۵) — «قفلِ لنگر» + ایتم‌هایِ واقعیِ طراح + هدرِ فشرده + گاوصندوقِ تنظیمات:** لایهٔ پایانیِ `public/crm-v12.20.0.js` (پس ازِ باندل؛ جانشینِ `crm-v12.19.0.js`). ۱) **قفلِ لنگر (anchor lock):** ترتیبِ واقعیِ DOMِ هر فرم در `CRM_V12200_ANCHORS` ذخیره می‌شود؛ اگر کدِ دیگری فیلدی را جابه‌جا کند (ریشهٔ اصلی: `saveDesignerField` گروه را `remove()` و `renderCustomFieldsInForm` نمونهٔ تازه را به **انتهای گرید** می‌چسباند، و `applyFullFormLayout` با «نیتِ مدیر» `grid.appendChild(container)` می‌کند) همان لحظه با کمینه جابه‌جایی به خانهٔ اصلی برمی‌گردد؛ کادرِ «📍 لوکیشن و نقشه داروخانه» چون «فیلد» نیست هرگز تکان نمی‌خورد؛ **تنها «شماره ترتیب در فرم» جایِ فیلد را عوض می‌کند** (بازسازیِ لنگر فقط در همان خانه‌هایی که فیلد نشسته‌اند). ۲) **همهٔ ایتم‌هایِ طراح واقعاً می‌نشینند:** «فاصله نسبت به فیلد قبلی/بعدی (میلی‌متر)» با تبدیلِ `mm×96/25.4` به پیکسل (هم `margin-inline-*` هم `margin-right/left` برایِ RTL، با `!important`)، عرض و ارتفاعِ پیکسلی روی خودِ ورودی، «شماره سطر» → `grid-row`، «زیرِ هم» → `grid-column:1/-1`، «داخل کدام کادر؟» (که مقدارِ کشویش در باندل دور ریخته می‌شد) ذخیره و اعمال می‌شود، «نمایش در فرم»، «وابسته به فیلد»، ستارهٔ الزام و «افزودن لحظه‌ای گزینه». ۳) **هدرِ فشردهٔ تک‌ردیفه:** ساعتِ **بی‌پرش** (`.crm-clock-time{width:64px}` + `tabular-nums`، پس تپشِ ثانیه نه کادر را بزرگ می‌کند نه تاریخ را تکان می‌دهد)، تاریخ، **یک** برچسبِ نسخه (برچسبِ تکراریِ `#v20VersionBadge` پنهان شد)، نامِ کاربر، وضعیتِ آنلاین، زنگوله، پنلِ نماینده، تغییرِ رمز و خروج — همه در بالاترین ردیف با `padding:0.22rem`. ۴) **گاوصندوقِ تنظیمات:** فیلدها، متادیتایِ فیلدها، کادرها و ترتیب‌ها هر ۶۰ ثانیه (و پس از هر تغییر) در سه جا می‌روند: `CRM_SETTINGS_VAULT_V1` در localStorage (که ریشه‌پاک‌کن دیگر جارویش نمی‌کند)، IndexedDBِ `crmSettingsVault` (جدا از `crmV19` که purge حذفش می‌کند) و سرور (`/api/vault` روی Node و `api.php?p=vault` روی نت‌افراز)؛ هنگامِ بالاآمدنِ نسخهٔ تازه هر چه گم شده **خودکار برمی‌گردد** — مگر فیلدی که مدیر عمداً حذف کرده باشد (سنگ‌قبر). دکمه‌هایِ «🛡 پشتیبان» و «↩ بازیابی» در هدر. ۵) **`/api/cleanup?purge=1` دیگر `crm-live-data.json`/`crm-live-bulk.json`ِ دارایِ داده را حذف نمی‌کند.** تست: **۲۵۷/۲۵۷**.
>
