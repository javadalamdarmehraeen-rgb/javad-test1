...
> # 🏷️ نسخه‌ی جاریِ این ریپو (GitHub main): **12.23.0**
>
> **تغییراتِ 12.23.0 (نوبت 148) — نه شکایت، نه پاسخ:** ۱) **تابلوِ روانِ رویدادهایِ روزِ تقویمِ شمسی** زیرِ کادرِ بالا: نوارِ باریکِ فیروزه‌ای با حرکتِ **آهسته** (۱۲۰ ثانیه) و بیش از ۱۳۰ مناسبتِ ملی؛ امروز را با `Intl` شمسی می‌یابد و مناسبتِ همان روز را می‌گرداند (اگر نبود، رویدادهایِ همان ماه). ۲) **هدرِ عمودیِ گوشی مرتب شد:** در `≤640px` و portrait، دو ردیفِ تمیز — لوگو/اکشن‌ها بالا، کادرِ ساعت تمام‌عرض پایین — به‌جایِ به‌هم‌ریختگی. ۳) **نگهبانِ ویزیت:** سایهٔ محلیِ `v20ActiveVisit` در sessionStorage؛ اگر pullِ همگام‌سازی آن را پاک کرد، برمی‌گردد تا GPS به ثبت ادامه دهد؛ پس از «پایان ویزیت» آمارِ جلسهٔ تمام‌شده (مسافت/توقف/نقاط/ساعتِ شروع/پایان) در همان کادر می‌ماند. ۴) **نقشهٔ تردد مثلِ مسیریاب:** علاوه بر خطِ مسیر، همهٔ داروخانه‌ها/مطب‌هایِ داخلِ ۲۵۰ متریِ مسیر کارت می‌شوند با برچسبِ دائمیِ «✅ ویزیت شده»/«🔴 ویزیت نشده» + نشانِ «شروع/پایان تردد». ۵) **اکسلِ واقعیِ .xlsx برایِ گوشی:** به‌جایِ HTML-در-.xls، یک ZIP معتبرِ xlsx (با sheet راست‌به‌چپ) ساخته می‌شود — اندروید و iOS بی‌بهانه بازش می‌کنند. ۶) **خودبروزرسانیِ نت‌افراز:** `auto-update.php` آخرین Release را از GitHub می‌گیرد و بدونِ دست‌زدن به داده‌ها نصب می‌کند — مشکلِ «لوگو/کد در نت‌افراز عوض نشده» برای همیشه حل می‌شود (یک‌بار آپلودِ دستی، بعد یک کلیک). (تغییراتِ نت‌افراز→همه‌جا هم با همین خودبروزرسانی + همگامِ سه‌هابیِ موجود هم‌خوان می‌شود.) تست: **274/274**.
>
