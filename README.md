...
> # 🏷️ نسخه‌ی جاریِ این ریپو (GitHub main): **12.22.0**
>
> **تغییراتِ 12.22.0 (نوبت ۱۴۷) — پرش تمام، لوگو یکپارچه، کادرِ ساعت خوانا:** ۱) **پایانِ «پرشِ زیاد»:** دو موتور (باندل و لایه) روی یک CSS `order` عددِ متفاوت می‌نوشتند و هر نوشتن دیگری را تحریک می‌کرد؛ حالا `harmonizeMetaOrders` شمارهٔ خانهٔ نهاییِ هر گره را در خودِ `formFieldMeta` می‌نویسد تا `applySavedLayout`ِ باندل «همان» عددها را حساب کند — دو موتور، یک خروجی، صفر جنگ. ۲) **کادرِ ساعت/تاریخ بلندتر:** پدینگ `0.42rem` و قلمِ `1.02rem`/`0.8rem` با پس‌زمینهٔ ملایم، ولی پهنایِ بیرونی ثابت (`302px`/`258px`) تا نپرد. ۳) **نشان‌های هدر میخکوب:** عرضِ آنلاین/آفلاین (`92px`)، زنگوله (`36px`) و نامِ کاربر ثابت شد تا تغییرِ متن هیچ عنصری را تکان ندهد. ۴) **لوگوی یکپارچه:** همهٔ `logo.png`، `favicon.png`، `apple-touch-icon.png`، `icon-*` و `icons/*` و manifest با نشانِ تازهٔ «طنین طب طاها» (هِگزاگونِ سبز + کپسول + کارخانه) جایگزین شدند. (مواردِ ۱ تا ۶ نوبتِ ۱۴۶ — تاریخِ بی‌ویرگول، قفلِ order+DOM، بستنِ منو، سینکِ صادقانه، سرستونِ فارسی، عددهایِ واقعیِ طراح — سرِ جایِ خودشان‌اند.) تست: **۲۶۹/۲۶۹**.
>
