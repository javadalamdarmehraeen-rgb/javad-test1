...
> # 🏷️ نسخه‌ی جاریِ این ریپو (GitHub main): **12.18.0**
>
> **به‌روزرسانی 2026-09-06 (نوبت ۱۳۵):** اگر در هر کجا (Render، نت‌افراز، ZIP روی دیسک) نسخه‌ای پایین‌تر از `12.18.0` می‌بینید، آن یک deployِ کهنه است؛ سورسِ درست همین ریپو است.
>
> **وضعیت انتشار (نوبت ۱۳۶):** کامیتِ `f81ac40` (=12.18.0) رویِ شاخهٔ PR #8 push شده است. با یک کلیک «Merge pull request» در GitHub، Render و ndcohub خودکار به `12.18.0` می‌رسند؛ برای `mehraeinpharma.ir` ZIPِ همین نسخه را در پنل آپلود کنید. تا قبلِ Merge، همه‌جا همان `12.16.0` اجرا می‌شود.
>
> - تولید (Render): `https://javad-test1.onrender.com`
> - نت‌افراز: `https://mehraeinpharma.ir` + `https://ndcohub.com`
> - **بررسی نسخه:** بالا-راستِ برنامه یا `https://جایگاه/api/health` — عددِ `version` باید `12.18.0` باشد.
>
...
> **راهنمای بررسی نسخه جاری:** قبل از هر بررسی GitHub، فایل [`GITHUB_REVIEW_HANDOFF.md`](./GITHUB_REVIEW_HANDOFF.md) و سپس [`PROJECT_GRAPH.md`](./PROJECT_GRAPH.md) را بخوانید. نسخه فعال سورس `12.18.0` است؛ ورودی runtime اصلی `server.js + public/` است و اسکلت Next.js پوشه `src/` مسیر production فعلی نیست.
>
