...
> # 🏷️ نسخه‌ی جاریِ این ریپو (GitHub main): **12.18.2**
>
> **به‌روزرسانی 2026-09-06 (نوبت ۱۳۹):** اگر در هر کجا (Render، نت‌افراز، ZIP روی دیسک) نسخه‌ای پایین‌تر از `12.18.2` می‌بینید، آن یک deployِ کهنه است؛ سورسِ درست همین ریپو است.
>
> **تغییراتِ 12.18.2 (سه خواستهٔ این نوبت):**
...
> - **بررسی نسخه:** بالا-راستِ برنامه یا `https://جایگاه/api/health` — عددِ `version` باید `12.18.2` باشد.
>
...
> **راهنمای بررسی نسخه جاری:** قبل از هر بررسی GitHub، فایل [`GITHUB_REVIEW_HANDOFF.md`](./GITHUB_REVIEW_HANDOFF.md) و سپس [`PROJECT_GRAPH.md`](./PROJECT_GRAPH.md) را بخوانید. نسخه فعال سورس `12.18.2` است؛ ورودی runtime اصلی `server.js + public/` است و اسکلت Next.js پوشه `src/` مسیر production فعلی نیست.
>
