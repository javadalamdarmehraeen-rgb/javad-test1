...
> # 🏷️ نسخه‌ی جاریِ این ریپو (GitHub main): **12.25.0**
>
> **تغییراتِ 12.25.0 (نوبت 152) — نه شکایت، نه پاسخ:** ۱) **یک ستاره:** «**نام داروخانه*** *» تمام شد؛ `*`هایِ اضافیِ متنِ برچسب پاک و فقط یک spanِ ستاره نگه داشته می‌شود. ۲) **تابلویِ روان:** یک دور **۶۰ ثانیه** و هر رویداد **یک‌بار** (کپیِ دومِ حلقه حذف شد). ۳) **نام‌هایِ خراب (â€Žâ€Ž…) پاک شد** و رشتهٔ اقلام فشرده و بی‌تکرار: «سافت ژل امگا 3 (100 کالا / 10 جایزه)». ۴) **کالایِ حذف‌شده برنمی‌گردد:** سنگ‌قبرِ نام + حذفِ تکراری‌ها (mergeِ رندر دیگر زنده‌شان نمی‌کند). ۵) **هر فیلدِ تازهٔ داروخانه‌ها در سفارشات هم ساخته می‌شود** (همان عنوان/نوع/گزینه/ترتیب) و با حذفِ مبدأ، آینه هم می‌رود. ۶) **فقط یک کادرِ «جایگذاری خودکارِ داروخانه»** — کادرِ دوم پنهان شد (نه حذف)، پس پرش و ظهورِ پیاپی تمام. ۷) **شمارهٔ ترتیبِ فرم و لیست دوباره کار می‌کند:** نگهبان دیگر ترتیبِ کهنه را برنمی‌گرداند (ضبطِ دوبارهٔ لنگر پس از تغییر). ۸) **پشتیبانِ خودکار هر ۵ دقیقه** (به‌جایِ ۳۰ ثانیه) + هوکِ `saveState` و برچسبِ ساعت در هدر. ۹) **«🗺 نمایش تردد» کار می‌کند:** تب باز می‌شود، نقشه ساخته می‌شود، مسیرِ مبدأ→مقصد کشیده می‌شود و نتیجه در کادرِ وضعیت دیده می‌شود. تست: **290/290**.
>
