...
> # 🏷️ نسخه‌ی جاریِ این ریپو (GitHub main): **12.24.0**
>
> **تغییراتِ 12.24.0 (نوبت 150) — شش شکایت، شش پاسخ:** ۱) **تابلوی روان فریز شد:** نوارِ رویدادها «داخلِ» هدرِ چسبان (`.app-header`) می‌نشیند، پس با اسکرول همراهِ هدر می‌ماند و تکان نمی‌خورد. ۲) **پشتیبانِ خودکارِ دیدنی:** هر **30** ثانیه + پس از هر `saveState` (با 2.5s تأخیر) و هر ذخیرهٔ طراح؛ برچسبِ «🛡 HH:MM:SS» کنارِ ساعتِ هدر زمانِ آخرین پشتیبان را نشان می‌دهد. ۳) **گم‌نشدنِ فیلدها پایان یافت:** هوکِ `saveState` + بازهٔ 30s + گاوصندوق (localStorage+IndexedDB+سرور)؛ فایل‌های داده هرگز داخلِ ZIP نیستند، پس آپلودِ نسخه دادهٔ زنده را بازنویسی نمی‌کند. ۴) **انتقالِ رندر ↔ نت‌افراز:** کارتِ «🔁 انتقالِ داده» در تبِ عیب‌یابی با سه دکمه: کپیِ کامل از رندر، ارسالِ کامل به نت‌افراز، ارسالِ کامل به رندر (با گزارشِ واقعیِ HTTP). ۵) **شروع/پایانِ ویزیت واقعاً کار می‌کند:** لایه مالکِ کاملِ دو دکمه است (capture + `stopImmediatePropagation`)، اجازهٔ GPS را صریح می‌خواهد، وضعیت را در `#visitStatusBox` «دیدنی» می‌نویسد (🛰/⛔/⚠️)، هر 5s با `getCurrentPosition` بیمه می‌کند و پایانِ جلسه را در `repRoutes`+`visitTracks`+کادرِ آمار+نقشهٔ تردد می‌نشاند. ۶) **ریشهٔ «1,405»:** هاست‌ها کدِ قدیمیِ `main` را اجرا می‌کردند چون PR ادغام نشده بود؛ در این نوبت PR #9 به `main` ادغام می‌شود تا رندر کدِ درست (تاریخِ بی‌ویرگول، لوگویِ تازه، ویزیتِ سالم) را بگیرد. تست: **280/280**.
>
