...
> **راهنمای بررسی نسخه جاری:** قبل از هر بررسی GitHub، فایل [`GITHUB_REVIEW_HANDOFF.md`](./GITHUB_REVIEW_HANDOFF.md) و سپس [`PROJECT_GRAPH.md`](./PROJECT_GRAPH.md) را بخوانید. نسخه فعال سورس `11.61.0` است؛ ورودی runtime اصلی `server.js + public/` است و اسکلت Next.js پوشه `src/` مسیر production فعلی نیست.
>
> **وضعیت انتشار در 2026-08-25 (نوبت ۸۰):** سورس `11.61.0` آماده push کاربر است — اصلاح ریشه‌ای «فیلدهای سه تب داروخانه/پزشک/سفارش»: سه حلقه خودتغذی MutationObserver (مرتب‌سازی ستون لیست، قفل موقعیت فرم سفارش، بازنویسی‌های چیدمان بدون idempotent) بسته شدند؛ جهش DOM بیکار از هزاران در ثانیه به صفر رسید؛ انتخاب کشویی/تایپ/ثبات چیدمان End-to-End تأیید شد و ۸۲/۸۲ تست سبز است. production پس از push شما به 11.61.0 می‌رسد. جزئیات در فایل handoff.
